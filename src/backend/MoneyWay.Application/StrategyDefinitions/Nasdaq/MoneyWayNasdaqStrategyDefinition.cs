    ...
    public static StrategyDefinition Instance { get; } = new(
        ...
        [
            ...
            Rule("NQ-LIQ-003", "Liquidity take required", "Sweep", 70, true, RuleDefinitionStatus.Confirmed,
                "From the 08:30 operational start and before 11:00 America/Bogota, a valid liquidity-take event occurs when price exceeds a selected relevant High or goes below a selected relevant Low. This prerequisite enables downstream 5M progression but does not confirm Step 4 or entry. Before Step 4, a subsequent same-side extension may remain within the same single pending setup rather than create a parallel setup."),
            ...
            Rule("NQ-M5-001", "Inversion alternatives", "5M inversion", 110, true, RuleDefinitionStatus.Confirmed,
                "Within an active pre-entry setup before 11:00 America/Bogota and only after its valid Step-3 liquidity take, Step 4 requires 5M Structural Change OR IFVG, whichever occurs first; neither alternative bypasses the separate Step-5 FVG confirmation. An isolated 5M pattern without that preceding progression is not a valid strategy Step 4. Before confirmation, same-side continuation may update the active structural reference while Step 4 remains pending; exact structural and IFVG geometry remain unresolved."),
            ...
            Rule("NQ-M1-003", "Entry mechanics", "Entry", 190, false, RuleDefinitionStatus.Unresolved,
                "Final pre-entry eligibility requires the valid preceding Steps 1 through 5 plus the Step-6 1M countertrend pullback and directional realignment to complete in chronological order before 11:00 America/Bogota. A late isolated 1M signal cannot revive a cancelled or expired setup; runtime gate-status mapping and order mechanics remain unresolved."),
