    ...
    public static StrategyDefinition Instance { get; } = new(
        ...
        [
            ...
            Rule("NQ-LIQ-003", "Liquidity take required", "Sweep", 70, true, RuleDefinitionStatus.Confirmed,
                "From the 08:30 operational start and before 11:00 America/Bogota, a valid liquidity-take event occurs when price exceeds a selected relevant High or goes below a selected relevant Low. This Step-3 prerequisite enables downstream progression only within its active pre-entry setup; it does not confirm Step 4 or entry. Before Step 4, a subsequent same-side extension may remain within that single setup rather than create a parallel setup. Target selection occurs outside this rule: a session target unavailable before the operational setup context may require conceptual structural fallback, while consumption of an already-selected target after setup activation is a separate lifecycle cancellation condition. This rule neither selects a fallback nor detects target touch or cancellation. Target consumption after 08:30 but before Step-3 activation remains unresolved."),
