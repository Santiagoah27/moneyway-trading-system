...
public sealed class MoneyWayNasdaqStrategyDefinitionTests
{
    ...
    public void FourHourContextMetadataMatchesAuditedSpecification()
    {
        ...
        Assert.Contains("confirms the new LL and validates the preceding LH only from that causal AsOfUtc", context.Description, StringComparison.Ordinal);
        Assert.Contains("Wick-only penetration and an open candle do not confirm it", context.Description, StringComparison.Ordinal);
        Assert.Contains("candidate symmetry hypotheses, not confirmed deterministic rules", context.Description, StringComparison.Ordinal);
        Assert.Contains("Complex candidate-LH selection, highest-high replacement", context.Description, StringComparison.Ordinal);
    ...
    public void ReverifiedConceptsRemainExplicitWithoutClaimingDeterministicGeometry()
    {
        ...
        Assert.Contains("validates the preceding LH only from that causal AsOfUtc", structuralLiquidity.Description, StringComparison.Ordinal);
        ...
        Assert.Contains("candidate symmetry hypotheses, not confirmed deterministic rules", structuralLiquidity.Description, StringComparison.Ordinal);
        Assert.Contains("Complex candidate-LH selection, highest-high replacement", structuralLiquidity.Description, StringComparison.Ordinal);
        ...
        Assert.Contains("demonstrated complex candidate-LH reconstruction", structuralLiquidity.Description, StringComparison.Ordinal);
        ...
        Assert.Contains("validates the preceding LH only from that causal AsOfUtc", target.Description, StringComparison.Ordinal);
        Assert.Contains("Wick-only penetration or an open candle does not confirm it", target.Description, StringComparison.Ordinal);
        Assert.Contains("candidate symmetry hypotheses, not confirmed deterministic rules", target.Description, StringComparison.Ordinal);
        Assert.Contains("Complex candidate-LH selection, highest-high replacement", target.Description, StringComparison.Ordinal);
        ...
        Assert.Contains("demonstrated complex candidate-LH reconstruction", target.Description, StringComparison.Ordinal);
