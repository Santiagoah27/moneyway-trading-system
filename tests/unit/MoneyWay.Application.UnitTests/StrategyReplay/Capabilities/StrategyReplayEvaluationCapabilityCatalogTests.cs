...
public sealed class StrategyReplayEvaluationCapabilityCatalogTests
{
    ...
    public void NasdaqTakeProfitCapabilityReasonNamesOnlyCurrentSpecificationBlockers()
    {
        ...
        Assert.Contains("Candidate-HL selection and its body anchor are partially defined", declaration.Reason, StringComparison.Ordinal);
        Assert.Contains("preliminary or intermediate lows are discarded", declaration.Reason, StringComparison.Ordinal);
        Assert.Contains("deepest low forming the base of the impulse", declaration.Reason, StringComparison.Ordinal);
        Assert.Contains("one- or two-candle pauses inside that impulse do not create another HL", declaration.Reason, StringComparison.Ordinal);
        Assert.Contains("lowest Open or Close among its candle bodies", declaration.Reason, StringComparison.Ordinal);
        Assert.Contains("not an isolated wick", declaration.Reason, StringComparison.Ordinal);
        Assert.Contains("usable only from the causal confirming close AsOfUtc", declaration.Reason, StringComparison.Ordinal);
        Assert.Contains("Candidate-LH selection remains unresolved", declaration.Reason, StringComparison.Ordinal);
        Assert.Contains("Exact correction and turn boundaries or segmentation", declaration.Reason, StringComparison.Ordinal);
        Assert.Contains("equal-coordinate body tie-breaking", declaration.Reason, StringComparison.Ordinal);
        Assert.Contains("other structural-point selection and coordinates", declaration.Reason, StringComparison.Ordinal);
        Assert.Contains("pivot/lookback rules if required", declaration.Reason, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("Initial candidate-turn detection", declaration.Reason, StringComparison.Ordinal);
        Assert.DoesNotContain("candidate swing geometry", declaration.Reason, StringComparison.Ordinal);
        Assert.DoesNotContain("exact structural candle/body selection and coordinate", declaration.Reason, StringComparison.Ordinal);
