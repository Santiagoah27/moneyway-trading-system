...
public sealed class MoneyWayNasdaqTradingWindowStartEvaluatorTests
{
    ...
    private readonly MoneyWayNasdaqTradingWindowStartEvaluator evaluator = new();
    ...
    [Theory]
    [InlineData(2026, 1, 15, 12, 30, 0, RuleEvaluationResult.Waiting)]
    [InlineData(2026, 1, 15, 13, 0, 0, RuleEvaluationResult.Waiting)]
    [InlineData(2026, 1, 15, 13, 29, 59, RuleEvaluationResult.Waiting)]
    [InlineData(2026, 1, 15, 13, 30, 0, RuleEvaluationResult.Passed)]
    [InlineData(2026, 1, 15, 14, 0, 0, RuleEvaluationResult.Passed)]
    [InlineData(2026, 1, 15, 17, 0, 0, RuleEvaluationResult.Passed)]
    [InlineData(2026, 7, 15, 13, 29, 0, RuleEvaluationResult.Waiting)]
    [InlineData(2026, 7, 16, 13, 31, 0, RuleEvaluationResult.Passed)]
    public void ConvertsUtcToBogotaAndAppliesOnlyInclusiveStartBoundary(
        int year, int month, int day, int hour, int minute, int second, RuleEvaluationResult expected)
    {
        var context = Context(new DateTimeOffset(year, month, day, hour, minute, second, TimeSpan.Zero));

        var decision = evaluator.Evaluate(context);

        Assert.Equal(expected, decision.Result);
        Assert.Equal(expected == RuleEvaluationResult.Waiting
    ...
    {
        var at = new DateTimeOffset(2026, 3, 10, 13, 30, 0, TimeSpan.Zero);
    ...
    {
        var waiting = Context(new DateTimeOffset(2026, 2, 1, 13, 0, 0, TimeSpan.Zero));
        var passed = Context(new DateTimeOffset(2026, 2, 1, 14, 0, 0, TimeSpan.Zero));

        Assert.Equal(RuleEvaluationResult.Passed, evaluator.Evaluate(passed).Result);
        Assert.Equal(RuleEvaluationResult.Waiting, evaluator.Evaluate(waiting).Result);
        Assert.Equal(RuleEvaluationResult.Waiting, evaluator.Evaluate(waiting).Result);
        Assert.Equal(RuleEvaluationResult.Passed, evaluator.Evaluate(passed).Result);
    }
    ...
    public void RejectsNullOrMismatchedStrategyContext()
    {
        ...
        var context = Context(new DateTimeOffset(2026, 1, 15, 13, 30, 0, TimeSpan.Zero), definition: new(
