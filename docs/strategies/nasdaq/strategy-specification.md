...
| `CANDIDATE_HL_SELECTION` | `PARTIALLY_DEFINED` | Deepest-low replacement, bullish reset/invalidation membership and collision destruction/exclusion are confirmed; remaining inclusivity, tie identity and bootstrap still prevent a complete detector. |
...
| `BULLISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | In-range continuity, strict new-High reset, same-candle bearish restart membership, prior-HL invalidation-candle exclusion and invalidation-dominant collision termination are confirmed; near-doji treatment, remaining inclusivity and bootstrap remain unresolved. |
...
| `CORRECTION_PHASE_BOUNDARIES` | `PARTIALLY_DEFINED` | Directional starts, exact-doji behavior, same-candle origin-extreme precedence, causal confirmations and mirrored terminal membership are confirmed; near-doji treatment, remaining inclusivity and bootstrap remain undefined. |
| `TURN_MEMBERSHIP_SEGMENTATION` | `PARTIALLY_DEFINED` | Both directions now have confirmed in-range continuity, reset/start inclusion, invalidation exclusion and collision exclusion; near-doji treatment, remaining inclusivity, bootstrap and broader impulse/correction edges still prevent complete segmentation. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Mirrored in-range and terminal membership, collision precedence, color/doji continuity, strict origin-extreme reset and prior-structure body-close invalidation join the confirmed candidate and causal-validation rules; near-doji treatment, remaining inclusivity, bootstrap and other structural identity/coordinate edges remain non-reproducible. |
