...
| `BULLISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Retrospective window includes exact-doji no-start and higher-High ceiling precedence before the first bearish-body close, then correction observations, impulsive expansion and confirming closed-body break; near-doji, gaps, boundary inclusivity and turn membership remain unresolved. |
...
| `CORRECTION_PHASE_BOUNDARIES` | `PARTIALLY_DEFINED` | Both directional starts, exact-doji no-start, both same-candle origin-extreme precedence cases and both causal ends are confirmed; near-doji, gaps, exact inclusivity and turn segmentation remain undefined. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Confirming-candle roles, exact-doji neutral/no-start behavior, the single-candle `Open` origin coordinate and both same-candle origin-extreme precedence cases are defined alongside materially narrowed bullish/bearish candidate rules; near-doji, gaps, exact inclusivity, segmentation and bootstrap remain non-reproducible. |
