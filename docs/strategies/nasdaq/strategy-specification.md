...
| `BEARISH_CORRECTION_START` | `CONFIRMED` | After the prior LL and cessation of new lows, the first bullish candle that begins upward displacement starts the bearish correction. |
...
| `BULLISH_CORRECTION_START` | `PARTIALLY_DEFINED` | The first bearish-body candle starts the correction after any same-candle higher `High` updates the origin ceiling. A distinct preceding HH candle, including an exact doji at that extreme, is excluded; near-doji treatment and other start-boundary cases remain undefined. |
...
| `BULLISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Distinct HH-candle pre-start exclusion, in-range continuity, strict new-High reset, same-candle bearish restart membership, prior-HL invalidation-candle exclusion and invalidation-dominant collision termination are confirmed; near-doji treatment, remaining inclusivity and initial-anchor selection remain unresolved. |
...
| `BEARISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Start with distinct LL-candle pre-start exclusion, in-range membership, strict new-Low reset, same-candle bullish restart, prior-LH invalidation, invalidation-dominant collision, causal `Close < prior LL` end, highest-candidate replacement and confirming-candle exclusion are confirmed; near-doji treatment, remaining inclusivity and initial-anchor selection still prevent a complete window detector. |
...
| `CORRECTION_PHASE_BOUNDARIES` | `PARTIALLY_DEFINED` | Directional starts, exact-doji behavior, same-candle origin-extreme precedence, causal confirmations and mirrored terminal membership are confirmed; near-doji treatment, remaining inclusivity and initial-anchor selection remain undefined. |
| `TURN_MEMBERSHIP_SEGMENTATION` | `PARTIALLY_DEFINED` | Both directions have confirmed distinct-extreme and pre-start-doji exclusion, in-range continuity, reset/start inclusion, invalidation exclusion and collision exclusion; near-doji treatment, remaining inclusivity, initial-anchor selection and other impulse/correction edges still prevent complete segmentation. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Exact equal-price identity, distinct-extreme and pre-start-doji exclusion, mirrored in-range and terminal membership, collision precedence, color/doji continuity, strict origin-extreme reset and prior-structure body-close invalidation join the confirmed candidate and causal-validation rules; near-equal tolerance, near-doji treatment, remaining inclusivity, initial-anchor selection and other coordinate edges remain non-reproducible. |
