...
Direct human review of Video 2 approximately `01:40–01:55`, `03:15–04:50`, `06:30`, `11:10–11:25`, `12:20` and `13:15–13:40`, plus Video 3 approximately `07:25–07:46` and `09:00`, partially defines the bullish correction window used for candidate-HL selection and confirms exact equal-price structural identity. From the prior HH peak, once the bullish impulse stops producing HHs, the first bearish candle starts the correction and the prior HH remains its upper structural boundary. The correction remains active through internal zigzags and deeper lows until a formally closed candle has `Close > prior HH structural level`; only at that causal close does the deepest candidate become a validated HL. The candidate is the deepest low/base of the correction and of the impulse that produces the break; preliminary/intermediate lows are replaced by a deeper low, and one- or two-candle pauses inside that impulse do not create another HL. For the supplied turn, the structural reference uses the lowest `Open` or `Close` among its candle bodies, not an isolated wick. Multiple member candles at that exact resulting body price support one structural price level; no candle ownership or first/last/timestamp tie-break is required. Near-doji thresholds and remaining prior-HH boundary inclusivity remain unresolved, as does any tolerance for merging merely near-equal prices.

Direct human review of Video 2 approximately `09:25–10:15` and `12:45–13:40`, especially `13:00–13:10`, confirms general in-range membership for an already-active bullish correction. Bearish, bullish and exact-doji candles, internal oscillations and consolidations remain in the same correction turn while no terminal/reset event occurs; body-color alternation does not fragment the turn or create parallel HL candidates. A strict `High > current correction-origin ceiling` first resets the active correction, discards its current candidate HL and updates the ceiling to that higher price extreme. The same closed observation is then classified by body direction: when `Close < Open`, that reset candle is excluded from the old turn and included as the first member of the new bullish-correction turn; when `Close == Open` or `Close > Open`, the reset still occurs but no new correction starts until a later qualifying bearish-body candle. This same-observation ordering uses closed OHLC only and does not reconstruct an intrabar path. The updated ceiling is not thereby a validated structural HH. Independently, a formally closed `Close < prior validated HL structural reference` invalidates the bullish structure and terminates candidate-HL pursuit; equality and wick-only penetration do not. The causal invalidation candle is excluded from the destroyed correction turn and its candidate-HL geometry, cannot change that candidate's `StructuralPrice` or `ProtectionAnchor`, and supplies evidence only for a new bearish impulse rather than an automatically validated structural LL. Membership is fixed at the candle's closed boundary and cannot be reinterpreted by later candles.
...
|---:|---|---|---|---|---|---|
| 1. 4H HH/LL context | `confirmed` + `human_validation_required` | Closed 4H candles | Direction inherited from the latest confirmed break, active structural pair, scenario and permitted direction reviewed | Blocks all lower stages | Yes | Initial-anchor selection, historical scan termination, exact body coordinate and retracement boundaries |
...
When facing a clean chart, the mentor visually/contextually starts from an evident High or Low that originated the current directional leg and reconstructs the recent push, retracement, structural break, next push and current structure. This confirms the initial-visible-anchor concept, but statements such as “I already see it clearly” and “I will draw from here” do not define a machine-reproducible selector over arbitrary raw 4H candles. Deterministic evaluation must not substitute viewport size, zoom, screen width or an arbitrary number of loaded candles for that missing rule.

The mentor defines no fixed candle count, day count, session count, time window, pivot period or numeric lookback formula for this bootstrap. `H4_FIXED_LOOKBACK_RULE = CONFIRMED_NONE`; this excludes invented fixed or ATR-derived windows, but it does not define an unbounded historical scan or its reproducible termination boundary.

Once valid structural references are already established, direction inherits from the latest confirmed strict body-close structural break: a formally closed body above the valid upper structural reference establishes bullish context, while a formally closed body below the valid lower structural reference establishes bearish context. Wick-only penetration and equality do not establish that break, and current candle colors alone do not infer trend. After an upper break, the resulting HH-side extreme and the originating valley/turn that supplied the HL-side reference become the active pair; after a lower break, the resulting LL-side extreme and originating peak/turn that supplied the LH-side reference become the active pair. Older points remain auditable, but operational context compresses to that latest HH+HL or LL+LH pair until structure changes again.

This post-anchor progression does not solve initialization circularity: “find the latest structural level that was broken” is deterministic only after the historical structural levels being compared already exist. The source still supplies no raw-candle algorithm for selecting the first structural anchor/reference. Asia/London session highs/lows, weekly open and previous-day extrema remain operational or session context references and are not mandatory H4 bootstrap seeds.
...
- A strict `High > current correction-origin ceiling` during an active bullish correction resets that correction. The prior candidate HL ceases to be current and the ceiling updates to the new higher price extreme before the same closed candle's body is classified. If `Close < Open`, that candle is excluded from the old correction and included as the first member of the new bullish-correction turn; no second bearish candle is required. If `Close == Open` or `Close > Open`, the reset occurs but the new correction does not start, so a later qualifying bearish-body candle is required. The reset does not validate a structural HH: the independent formally closed body-break rule still applies. This is a same-observation OHLC rule and does not infer a finer intrabar path.
...
- Implementation boundary: existing body-direction and extreme calculators expose the needed observations, and the lifecycle calculator clears an active turn on `ResetAndAwaitCorrectionStart`. They do not retain an inactive waiting phase or transition a normal or reset-and-await sequence into a new turn on its first opposite-body candle. Caller-selected geometry is compatible with the exclusion rule only when the caller supplies the correct turn; no automatic pre-start segmentation is established.
...
Historical automation of `NQ-H4-001` therefore requires reconstruction using only information observable through each `AsOfUtc`. Before the causal bullish or bearish close, the candidate remains unvalidated; later observations cannot alter an earlier historical evaluation or terminal-candle membership. The source defines both correction starts and causal ends, exact-doji neutral/no-start behavior, both origin-extreme precedence cases and their floor/ceiling updates, mirrored in-range membership, mirrored reset and invalidation semantics, invalidation-dominant collisions, terminal-candle membership, confirming-candle exclusion and validation role, the single-candle `Open` origin coordinate, exact equal-price structural identity, direction from the latest confirmed break and compression to the latest active pair. It also directly confirms the bearish prior-LL origin, complex candidate-LH selection, highest-high replacement and selected-turn highest-body-edge LH coordinate, while partially defining the bullish and bearish correction windows, candidate-HL selection and body anchoring. It does not define a complete detector: initial-anchor selection from arbitrary raw candles, near-doji thresholds, remaining boundary inclusivity, near-equal-zone tolerance and coordinates for other structural-point classes remain unresolved or partial. No fixed lookback exists, so it must not be retained as an unresolved numeric-rule question.
...
| `BEARISH_CORRECTION_START` | `CONFIRMED` | After the prior LL and cessation of new lows, the first exact bullish body (`Close > Open`) starts the bearish correction; all prior waiting candles are excluded and no body-size threshold qualifies the start. |
...
| `H4_INITIAL_VISIBLE_ANCHOR_CONCEPT` | `CONFIRMED` | The visual review begins from an evident High/Low that originated the current directional leg. |
| `H4_INITIAL_VISIBLE_ANCHOR_SELECTION` | `HUMAN_VALIDATION_REQUIRED` | No raw-candle rule selects that first anchor independently of viewport, zoom, screen width or arbitrary loaded history. |
...
| `H4_RAW_CANDLE_INITIALIZATION` | `HUMAN_VALIDATION_REQUIRED` | Arbitrary raw 4H history cannot reproducibly establish the first structural reference without a human-approved anchor. |
...
| `CANDIDATE_HL_SELECTION` | `PARTIALLY_DEFINED` | Deepest-low replacement, exact equal-price identity, bullish reset/invalidation membership and collision destruction/exclusion are confirmed; remaining inclusivity, near-equal tolerance and initial-anchor selection still prevent a complete detector. |
| `BULLISH_CORRECTION_START` | `CONFIRMED` | The first exact bearish body (`Close < Open`) starts the correction after any same-candle higher `High` updates the origin ceiling. Every prior waiting candle is excluded, including distinct HH and exact-doji candles; no body-size threshold qualifies the start. Broader near-doji classification remains separately unresolved. |
...
| `BULLISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Pre-start waiting exclusion, in-range continuity, strict new-High reset, same-candle bearish restart membership, prior-HL invalidation-candle exclusion and invalidation-dominant collision termination are confirmed; near-doji treatment beyond the exact start comparison, remaining inclusivity and initial-anchor selection remain unresolved. |
...
| `BULLISH_CORRECTION_NEW_HIGH_RESET` | `CONFIRMED` | `High > current correction-origin ceiling` resets the active correction and updates the ceiling without validating a structural HH. |
| `BULLISH_CANDIDATE_RESET_ON_NEW_HIGH` | `CONFIRMED` | The candidate HL from the reset correction ceases to be current; subsequent search proceeds from the updated ceiling under existing start rules. |
...
| `BULLISH_NEW_HIGH_RESET_CANDLE_DUAL_ROLE` | `CONFIRMED` | A strict new-High candle first resets the old correction and updates its ceiling, then its closed body direction determines whether the same observation starts the new correction. |
| `BULLISH_RESET_BEARISH_CANDLE_NEW_TURN_MEMBERSHIP` | `CONFIRMED_INCLUDED` | A reset candle with `Close < Open` is excluded from the old turn and included as the first member of the new bullish-correction turn. |
...
| `BEARISH_CORRECTION_NEW_LOW_RESET` | `CONFIRMED` | `Low < current correction-origin floor` resets the active bearish correction and updates the floor without validating a structural LL. |
| `BEARISH_CANDIDATE_LH_RESET_ON_NEW_LOW` | `CONFIRMED` | A strict new-Low reset discards the old candidate LH; subsequent search uses the updated floor and confirmed start rules. |
| `BEARISH_NEW_LOW_RESET_CANDLE_DUAL_ROLE` | `CONFIRMED` | A new-Low candle first resets the old correction and updates its floor, then its exact body direction determines whether that observation starts the new correction. |
| `BEARISH_RESET_BULLISH_CANDLE_NEW_TURN_MEMBERSHIP` | `CONFIRMED_INCLUDED` | A reset candle with `Close > Open` is excluded from the old turn and included as the first member of the new bearish-correction turn. |
...
| `BEARISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Start with pre-start waiting exclusion, in-range membership, strict new-Low reset, same-candle bullish restart, prior-LH invalidation, invalidation-dominant collision, causal `Close < prior LL` end, highest-candidate replacement and confirming-candle exclusion are confirmed; near-doji treatment beyond the exact start comparison, remaining inclusivity and initial-anchor selection still prevent a complete window detector. |
...
| `CORRECTION_PHASE_BOUNDARIES` | `PARTIALLY_DEFINED` | Exact directional starts, exclusion of every pre-start waiting candle, exact-doji behavior, same-candle origin-extreme precedence, causal confirmations and mirrored terminal membership are confirmed; near-doji treatment beyond exact starts, remaining inclusivity and initial-anchor selection remain undefined. |
| `TURN_MEMBERSHIP_SEGMENTATION` | `PARTIALLY_DEFINED` | Both directions have confirmed exclusion for all pre-start waiting candles, in-range continuity, reset/start inclusion, invalidation exclusion and collision exclusion; near-doji treatment beyond exact starts, remaining inclusivity, initial-anchor selection and other impulse/correction edges still prevent complete segmentation. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Exact equal-price identity, all pre-start waiting-candle exclusion, mirrored in-range and terminal membership, collision precedence, color/doji continuity, strict origin-extreme reset and prior-structure body-close invalidation join the confirmed candidate and causal-validation rules; near-equal tolerance, near-doji treatment beyond exact starts, remaining inclusivity, initial-anchor selection and other coordinate edges remain non-reproducible. |
...
| Initial HL/LL/LH/HH candidate detection | `PARTIALLY_DEFINED` | Bullish candidate-HL window/deepest-low/base selection, price anchor and mirrored terminal membership are materially narrowed; exact-doji no-start, both same-candle origin-extreme precedence cases, bearish start/end, origin/causal validation, complex highest-candidate selection and selected-turn highest-body-edge coordinate are confirmed. No fixed lookback exists; near-doji treatment, remaining inclusivity, other class selection, first-anchor selection and historical scan termination remain undefined. |
