...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | The mentor identifies the relevant slowdown/turn retrospectively and rejects minor noise, but candle count, pivot geometry, retracement boundaries and lookback are not reproducible. |
| `EXACT_STRUCTURAL_PRICE_COORDINATE` | `UNRESOLVED` | Body-zone marking is confirmed, but candle selection, Open versus Close and reduction to one decimal are undefined. |
...
| Candle/body used to place the line | `PARTIALLY_DEFINED` | Bodies are confirmed; candle selection and Open/Close/other body coordinate are unresolved. |
...
| Initial HL/LL/LH/HH candidate detection | `PARTIALLY_DEFINED` | Slowdown/turn selection is visual; no pivot, lookback, candle count or geometry may be inferred. |
