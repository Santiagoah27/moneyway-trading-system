...
The latest direct review of Video 3 approximately `03:10–03:25` and `08:50–10:50`, together with Video 1 approximately `10:10–10:30`, confirms that coincident Asia/London candidates form one target and that a session target no longer available as a future objective falls back conceptually to the next relevant structural 1H/4H point. No title, URL or transcript wording is inferred from these approximate references.
...
- A session target already consumed before it can become the active setup's future objective is a target-selection input: the source falls back conceptually to the next relevant structural 1H/4H point. This is distinct from cancellation and does not define a deterministic structural-point selector. The behavior when consumption occurs after 08:30 but before any Step-3 activation remains unresolved.
...
Manual source-video re-verification confirms the primary session candidates for this role: Asia High and London High for buys, and Asia Low and London Low for sells. When two future-valid levels differ, the immediate target is the first relevant level price encounters in the expected direction of travel. When their prices are equal, they collapse into one unique target; the reviewed equal-target scenario treats that level as final Take Profit. If a relevant session target was already consumed before the operational period or is otherwise unavailable as a future objective, the source falls back to the next relevant structural 1H/4H point identified in Step 2. This hierarchy does not define an automatic structural detector, 1H/4H priority or tie-break. It also does not make arbitrary structural points coequal primary candidates while valid session targets remain available.

Exact quote-level touch of the selected absolute price is sufficient; no body close, candle close, chart timeframe or tolerance is required. One reviewed example around Video 3 `13:25–13:40` supports complete exit when its selected level is touched, and the latest review confirms final-TP treatment for the reviewed coincident-target scenario. Neither establishes that every first distinct target is final TP or defines universal full-exit behavior. Historical same-candle ordering remains limited by market-data resolution. Structural fallback selection, the 08:30-to-Step-3 consumption edge, fixed ratio, partials, trailing and other final-target hierarchy remain unresolved source questions.
