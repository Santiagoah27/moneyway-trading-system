...
Direct human review of Video 2 approximately `05:40–06:15`, `06:40` and `13:20`, plus Video 3 approximately `04:25–04:35`, `04:30` and `07:15–07:45`, explicitly confirms the bearish causal-validation path and complex candidate-LH identity. A bearish retracement originates from the prior LL area, but its candidate high remains unvalidated while that LL lacks a confirmed break. Within one recognized bearish correction, a later higher candidate high replaces an earlier lower one; superseded intermediate highs do not survive as the LH candidate. Only a formally closed candle with `Close < prior LL structural level` confirms the new LL and validates that preceding highest candidate high as LH from that causal `AsOfUtc`; wick-only penetration and an open candle are insufficient. This confirms swing identity, not the decimal coordinate of the structural LH. The reviewed material does not define bearish doji/gap or candle-inclusivity edges, correction segmentation, confirming-candle scan participation, a structural LH body anchor, or a target/comparison coordinate. Separately, the sell risk example places Stop Loss above the highest wick/tail of the relevant LH. That SL/invalidation anchor does not establish that a structural LH coordinate for target selection, reconstruction or comparisons uses the wick.
...
| `BEARISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Prior-LL origin, causal closed `Close < prior LL` end and highest-candidate replacement are confirmed; doji/gap, start/inclusivity, scan participation and segmentation remain unresolved. |
| `CANDIDATE_LH_STRUCTURAL_PRICE` | `UNRESOLVED` | Identity confirmation does not specify the structural decimal used for targets, reconstruction or comparisons. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Bullish candidate-HL window/price plus bearish causal validation/origin and complex highest-candidate identity are materially narrowed; edge boundaries, bootstrap and structural coordinates remain non-reproducible. |
...
| Initial HL/LL/LH/HH candidate detection | `PARTIALLY_DEFINED` | Bullish candidate-HL window/deepest-low/base selection and price anchor are materially narrowed; bearish origin/causal validation plus complex highest-candidate selection are confirmed, while bearish correction edges and body anchor/coordinate remain unresolved or candidate. Doji/gap boundaries, turn membership, other class selection, pivots and lookbacks remain undefined. |
