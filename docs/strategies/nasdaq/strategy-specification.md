...
- If several included bodies share that same lowest `Open`/`Close`, the structural price is still unambiguous when the strategy needs only that price; the identity of a particular candle/timestamp remains unresolved. Exact doji no-start and mirrored terminal membership are confirmed; near-doji thresholds, gaps, remaining prior-HH/LL boundary inclusivity and the selected-candle identity remain unresolved.
...
| `CANDIDATE_HL_SELECTION` | `PARTIALLY_DEFINED` | Deepest-low replacement, bullish reset/invalidation membership and collision destruction/exclusion are confirmed; gaps, remaining inclusivity, tie identity and bootstrap still prevent a complete detector. |
| `BULLISH_CORRECTION_START` | `PARTIALLY_DEFINED` | The first bearish-body candle starts the correction after any same-candle higher `High` updates the origin ceiling; exact doji does not start it, while near-doji, gaps and exact HH-candle inclusivity remain undefined. |
...
| `BULLISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | In-range continuity, strict new-High reset, same-candle bearish restart membership, prior-HL invalidation-candle exclusion and invalidation-dominant collision termination are confirmed; near-doji, gaps, remaining inclusivity and bootstrap remain unresolved. |
...
| `CORRECTION_PHASE_BOUNDARIES` | `PARTIALLY_DEFINED` | Directional starts, exact-doji behavior, same-candle origin-extreme precedence, causal confirmations and mirrored terminal membership are confirmed; gaps, near-doji, remaining inclusivity and bootstrap remain undefined. |
| `TURN_MEMBERSHIP_SEGMENTATION` | `PARTIALLY_DEFINED` | Both directions now have confirmed in-range continuity, reset/start inclusion, invalidation exclusion and collision exclusion; gaps, near-doji, remaining inclusivity, bootstrap and broader impulse/correction edges still prevent complete segmentation. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Mirrored in-range and terminal membership, collision precedence, color/doji continuity, strict origin-extreme reset and prior-structure body-close invalidation join the confirmed candidate and causal-validation rules; near-doji, gaps, remaining inclusivity, bootstrap and other structural identity/coordinate edges remain non-reproducible. |
