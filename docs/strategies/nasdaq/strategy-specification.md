...
- If several included bodies share that same lowest `Open`/`Close`, the structural price is still unambiguous when the strategy needs only that price; the identity of a particular candle/timestamp remains unresolved. Exact doji no-start and bullish terminal membership are confirmed; near-doji thresholds, gaps, remaining prior-HH/LL boundary inclusivity, bearish terminal symmetry and the selected-candle identity remain unresolved.
...
The source now materially clarifies a retrospective bullish correction window: prior upside extreme → first bearish correction candle → correction observations → impulsive expansion → formally closed body above the prior structural HH. If a candle inside an active correction prints a strict new `High`, that observation first resets the old correction and updates the correction-origin ceiling. A bearish body on that same closed candle then starts the new correction with the candle included as its first member; a neutral or bullish body does not start the new turn. The updated ceiling is not thereby promoted to a validated structural HH, and no intrabar path is inferred. A prior-HL invalidation candle is excluded from the destroyed bullish turn and its candidate geometry and begins bearish impulse evidence without becoming a validated structural LL. The causal breakout candle remains excluded from correction membership and correction-candidate scanning. The window remains partially defined because near-doji thresholds, gap behavior, remaining boundary inclusivity, macro bootstrap and bearish terminal symmetry are not supplied. It defines the lowest `Open`/`Close` price for a supplied candidate-HL turn, including a price-preserving equal-coordinate tie, but not tied candle identity. On the bearish side, direct evidence confirms `prior LL → no further new lows → first bullish candle → active correction → formally closed Close < prior LL`, together with complex highest-candidate selection and replacement. If the first bullish candle prints a lower `Low`, that price extreme first updates the correction-origin floor and the candle's bullish close then starts the correction; the floor is not thereby promoted to a validated structural LL. An exact doji can update that floor but cannot supply the bullish-body start. Both origin-extreme precedence cases are confirmed without inferring a finer intrabar path or weakening the independent structural HH/LL body-close rules. The windows remain partially defined only at unresolved edge and segmentation cases. In the special single-candle case, the confirming candle's `Open` is the structural HL/LH origin coordinate and its `Close` is the causal validation event; it still does not become a correction candle. This does not generalize over the existing multi-candle selected-turn rules. No fixed pivot, fixed lookback or near-doji tolerance is inferred, and the buy/sell protection wicks remain separate from structural coordinates.
...
Historical automation of `NQ-H4-001` therefore requires reconstruction using only information observable through each `AsOfUtc`. Before the causal bullish or bearish close, the candidate remains unvalidated; later observations cannot alter an earlier historical evaluation or terminal-candle membership. The source defines both correction starts and causal ends, exact-doji neutral/no-start behavior, both origin-extreme precedence cases and their floor/ceiling updates, same-candle bearish reset/start membership, exclusion of a bullish invalidation candle from the destroyed turn and its candidate geometry, confirming-candle exclusion and validation role, and the single-candle `Open` origin coordinate. It also directly confirms the bearish prior-LL origin, complex candidate-LH selection, highest-high replacement and selected-turn highest-body-edge LH coordinate, while partially defining the bullish correction window, candidate-HL selection and body anchoring. It does not define a complete detector: bootstrap of the prior reference level, near-doji thresholds, gap rules, remaining boundary inclusivity, bearish reset/invalidation symmetry, tie identity and coordinates for other structural-point classes remain unresolved or partial.
...
| `BEARISH_RESET_INVALIDATION_SYMMETRY` | `UNRESOLVED` | No independently reviewed evidence establishes an active-bearish new-Low reset or prior-LH body-close invalidation as symmetric rules. |
...
| `BEARISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Prior LL, cessation of new lows, exact-doji no-start, first bullish-body start, simultaneous lower-Low precedence/floor update, causal closed `Close < prior LL` end, highest-candidate replacement and confirming-candle exclusion are confirmed; near-doji, gaps, exact LL-candle inclusivity and segmentation remain unresolved. |
...
| `CORRECTION_PHASE_BOUNDARIES` | `PARTIALLY_DEFINED` | Directional starts, exact-doji behavior, same-candle origin-extreme precedence, causal confirmations and bullish terminal membership are confirmed; gaps, near-doji, bearish symmetry and bootstrap remain undefined. |
| `TURN_MEMBERSHIP_SEGMENTATION` | `PARTIALLY_DEFINED` | Bullish in-range continuity, same-candle bearish reset inclusion, invalidation-candle exclusion and collision-candle exclusion are confirmed; remaining bullish edge cases and independently evidenced bearish segmentation remain unresolved. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Bullish in-range and terminal membership, collision precedence, color/doji continuity, new-High reset and prior-HL body-close invalidation join the confirmed candidate and causal-validation rules; near-doji, gaps, remaining inclusivity, bearish terminal symmetry and bootstrap remain non-reproducible. |
...
| Initial HL/LL/LH/HH candidate detection | `PARTIALLY_DEFINED` | Bullish candidate-HL window/deepest-low/base selection, price anchor and terminal membership are materially narrowed; exact-doji no-start, both same-candle origin-extreme precedence cases, bearish start/end, origin/causal validation, complex highest-candidate selection and selected-turn highest-body-edge coordinate are confirmed. Near-doji, gaps, remaining inclusivity, bearish terminal symmetry, other class selection, pivots and lookbacks remain undefined. |
