...
Direct human review of Video 2 approximately `13:15–13:40` and Video 3 approximately `07:40–07:46` partially defines candidate-HL selection. Within a human-delimited corrective phase containing several lows, pauses or internal oscillations, preliminary/intermediate lows are discarded: the candidate HL is the deepest low of the entire correction and must be the base from which the impulse that later closes above the prior HH originates. One- or two-candle pauses inside that impulse do not create another HL. For the selected turn, the structural reference uses the lowest `Open` or `Close` among the candle bodies assigned to that turn, not an isolated wick. The sources do not formalize correction boundaries, turn segmentation, equal-coordinate tie-breaking or the inverse candidate-LH selection rule.
...
- After an LL, the relevant high of the slowdown/retracement is provisional. It becomes a confirmed `LH` only when the later impulse originating from that candidate produces a formally closed body below the previous LL on the same relevant timeframe and forms the next LL. A decline that does not achieve that body-close break does not validate the candidate as structural LH.
- For candidate-HL selection, preliminary/intermediate lows inside one correction are discarded. The candidate is the deepest low of the entire human-delimited corrective phase and must be the base from which the impulse that later produces the confirming body-close break above the prior HH originates.
- Small pauses of one or two candles inside that impulse do not create a new candidate HL.
- The selected low remains only a candidate until the causal body-close break closes; only from that close `AsOfUtc` onward does it become a validated HL.
- The inverse causal validation of an already-supplied candidate LH remains confirmed, but the source intervals reviewed here do not establish the inverse candidate-LH selection rule. No symmetric selection algorithm is inferred.
...
- Exact correction boundaries, the candle set belonging to the turn/base and the identity selected when several bodies share the same extreme coordinate remain unresolved.
...
The source now materially clarifies how a candidate HL is selected and confirmed retrospectively after an already-supplied structural level is broken. It defines the lowest `Open`/`Close` body coordinate for a selected candidate-HL turn, but not the exact boundaries of the relevant correction, the candles belonging to the turn, equal-coordinate identity, candidate-LH selection or coordinates for other structural-point classes. No wick-based anchor, fixed pivot, fixed lookback or inverse selection formula is inferred.
...
Historical automation of `NQ-H4-001` therefore requires reconstruction using only information observable through each `AsOfUtc`. The source defines the causal confirmation order and partially defines candidate-HL selection/body anchoring, but not a complete detector: bootstrap of the prior reference level, exact correction/turn boundaries, candidate-LH selection, tie identity and coordinates for other structural-point classes remain unresolved.
...
| `CANDIDATE_LH_SELECTION` | `UNRESOLVED` | Causal LH validation is confirmed, but no direct evidence in the reviewed intervals supports an inverse candidate-selection rule. |
| `CORRECTION_PHASE_BOUNDARIES` | `UNRESOLVED` | No segmentation rule defines the first/last candle of the entire correction or the selected turn/base. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Candidate-HL selection is materially narrowed, but boundaries, segmentation, tie handling, bootstrap and candidate-LH selection remain non-reproducible. |
...
| Initial HL/LL/LH/HH candidate detection | `PARTIALLY_DEFINED` | Candidate-HL deepest-low/base selection is confirmed within a human-delimited correction; segmentation, candidate-LH/other class selection, pivots and lookbacks remain undefined. |
