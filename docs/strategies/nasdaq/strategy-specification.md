...
- If several included bodies share that same lowest `Open`/`Close`, the structural price is still unambiguous when the strategy needs only that price; the identity of a particular candle/timestamp remains unresolved. Exact doji no-start and mirrored terminal membership are confirmed; near-doji thresholds, remaining prior-HH/LL boundary inclusivity and the selected-candle identity remain unresolved.
...
| `CANDIDATE_HL_SELECTION` | `PARTIALLY_DEFINED` | Deepest-low replacement, bullish reset/invalidation membership and collision destruction/exclusion are confirmed; remaining inclusivity, tie identity and initial-anchor selection still prevent a complete detector. |
...
| `CANDIDATE_HL_PRICE_SELECTION` | `PARTIALLY_DEFINED` | Within the selected correction/turn, the deepest candidate's lowest body `Open`/`Close` determines the price; resolved terminal membership constrains the supplied turn, while remaining edge boundaries and tie identity remain open. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Mirrored in-range and terminal membership, collision precedence, color/doji continuity, strict origin-extreme reset and prior-structure body-close invalidation join the confirmed candidate and causal-validation rules; near-doji treatment, remaining inclusivity, initial-anchor selection and other structural identity/coordinate edges remain non-reproducible. |
...
| Candle/body used to place the line | `PARTIALLY_DEFINED` | A selected candidate-HL turn uses its lowest body `Open`/`Close`, while a selected candidate-LH turn uses its highest body `Open`/`Close`; bullish terminal membership is defined, while equal-coordinate identity and other structural-point coordinates remain unresolved. |
