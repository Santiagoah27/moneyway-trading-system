...
| `BULLISH_CORRECTION_START` | `PARTIALLY_DEFINED` | The first bearish-body candle starts the correction after any same-candle higher `High` updates the origin ceiling; exact doji does not start it, while near-doji treatment and exact HH-candle inclusivity remain undefined. |
...
| `BEARISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Start, in-range membership, strict new-Low reset, same-candle bullish restart, prior-LH invalidation, invalidation-dominant collision, causal `Close < prior LL` end, highest-candidate replacement and confirming-candle exclusion are confirmed; near-doji treatment, remaining inclusivity and initial-anchor selection still prevent a complete window detector. |
...
| `TURN_MEMBERSHIP_SEGMENTATION` | `PARTIALLY_DEFINED` | Both directions now have confirmed in-range continuity, reset/start inclusion, invalidation exclusion and collision exclusion; near-doji treatment, remaining inclusivity, initial-anchor selection and broader impulse/correction edges still prevent complete segmentation. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Exact equal-price identity, mirrored in-range and terminal membership, collision precedence, color/doji continuity, strict origin-extreme reset and prior-structure body-close invalidation join the confirmed candidate and causal-validation rules; near-equal tolerance, near-doji treatment, remaining inclusivity, initial-anchor selection and other coordinate edges remain non-reproducible. |
