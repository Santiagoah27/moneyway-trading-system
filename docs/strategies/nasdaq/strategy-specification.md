...
- After a prior LL, once the bearish impulse stops producing new lows, the first bullish candle that begins the upward displacement starts the bearish correction. This start is observable only from candles available through the current `AsOfUtc`; doji/`Close == Open`, gap, simultaneous bullish-body/new-low and exact LL-candle-inclusivity cases remain unresolved.
...
Historical automation of `NQ-H4-001` therefore requires reconstruction using only information observable through each `AsOfUtc`. Before the causal bullish or bearish close, the candidate remains unvalidated; later observations cannot alter an earlier historical evaluation. The source defines both correction starts and causal ends, excludes the confirming candle from correction membership/scanning, confirms its validation role and single-candle `Open` origin coordinate, directly confirms the bearish prior-LL origin, complex candidate-LH selection, highest-high replacement and selected-turn highest-body-edge LH coordinate, and partially defines the bullish correction window, candidate-HL price selection and body anchoring. It does not define a complete detector: bootstrap of the prior reference level, doji/gap rules, simultaneous contrary-body/new-extreme behavior, boundary inclusivity, turn membership, tie identity and coordinates for other structural-point classes remain unresolved or partial.
...
| `BEARISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Prior LL, cessation of new lows, first bullish-candle start, causal closed `Close < prior LL` end, highest-candidate replacement and confirming-candle exclusion are confirmed; doji/gap, simultaneous bullish-body/new-low behavior, exact LL-candle inclusivity and segmentation remain unresolved. |
...
| `CORRECTION_PHASE_BOUNDARIES` | `PARTIALLY_DEFINED` | Both directional starts and causal ends are materially narrowed; doji/gap handling, simultaneous contrary-body/new-extreme cases, exact inclusivity and turn segmentation remain undefined. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Confirming-candle roles and the single-candle `Open` origin coordinate are defined alongside materially narrowed bullish/bearish candidate rules; edge boundaries, exact segmentation and bootstrap remain non-reproducible. |
...
| Initial HL/LL/LH/HH candidate detection | `PARTIALLY_DEFINED` | Bullish candidate-HL window/deepest-low/base selection and price anchor are materially narrowed; bearish start/end, origin/causal validation, complex highest-candidate selection and selected-turn highest-body-edge coordinate are confirmed. Doji/gap and simultaneous contrary-body/new-extreme cases, exact inclusivity, turn membership, other class selection, pivots and lookbacks remain undefined. |
