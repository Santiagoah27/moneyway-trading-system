...
- A strict `High > current correction-origin ceiling` during an active bullish correction resets that correction. The prior candidate HL ceases to be current, the ceiling updates to the new higher price extreme, and a later correction search starts from that ceiling under the existing start rules. The reset does not validate a structural HH: the independent formally closed body-break rule still applies. The exact old/new-turn membership of the reset candle is not established, and no finer intrabar path is inferred.
- A formally closed candle with `Close < prior validated HL structural reference` invalidates the active bullish structure. The correction cannot produce or continue the same candidate HL after that causal close. `Close == prior HL` does not invalidate under this rule, and `Low < prior HL` with `Close >= prior HL` is wick-only penetration and also does not invalidate. Whether this terminal candle belongs to candidate-HL geometry remains unresolved.
...
- If several included bodies share that same lowest `Open`/`Close`, the structural price is still unambiguous when the strategy needs only that price; the identity of a particular candle/timestamp remains unresolved. Exact doji no-start behavior is confirmed; near-doji thresholds, gaps, exact prior-HH/LL boundary inclusivity, turn membership and the selected-candle identity remain unresolved.
...
Historical automation of `NQ-H4-001` therefore requires reconstruction using only information observable through each `AsOfUtc`. Before the causal bullish or bearish close, the candidate remains unvalidated; later observations cannot alter an earlier historical evaluation. The source defines both correction starts and causal ends, exact-doji neutral/no-start behavior, both origin-extreme precedence cases and their floor/ceiling updates, excludes the confirming candle from correction membership/scanning, confirms its validation role and single-candle `Open` origin coordinate, directly confirms the bearish prior-LL origin, complex candidate-LH selection, highest-high replacement and selected-turn highest-body-edge LH coordinate, and partially defines the bullish correction window, candidate-HL price selection and body anchoring. It does not define a complete detector: bootstrap of the prior reference level, near-doji thresholds, gap rules, boundary inclusivity, turn membership, tie identity and coordinates for other structural-point classes remain unresolved or partial.
...
| `CANDIDATE_HL_SELECTION` | `PARTIALLY_DEFINED` | Deepest-low replacement across one active bullish correction turn is confirmed; gaps, terminal/reset candle membership, remaining inclusivity and bootstrap still prevent a complete detector. |
...
| `BULLISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | In-range intermediate membership, color/doji continuity, strict new-High reset and prior-HL body-close invalidation are confirmed; near-doji, gaps, remaining inclusivity, reset/terminal candle membership and bootstrap remain unresolved. |
...
| `BULLISH_TERMINAL_RESET_CANDLE_MEMBERSHIP` | `UNRESOLVED` | The evidence does not define whether the new-High reset candle or prior-HL invalidation candle contributes to candidate geometry. |
...
| `CANDIDATE_HL_PRICE_SELECTION` | `PARTIALLY_DEFINED` | Within the selected correction/turn, the deepest candidate's lowest body `Open`/`Close` determines the price; terminal/reset membership and remaining edge boundaries are unresolved. |
...
| `CORRECTION_PHASE_BOUNDARIES` | `PARTIALLY_DEFINED` | Directional starts, exact-doji behavior, same-candle origin-extreme precedence, causal confirmations and bullish reset/invalidation are confirmed; gaps, near-doji, terminal membership, bearish symmetry and bootstrap remain undefined. |
| `TURN_MEMBERSHIP_SEGMENTATION` | `PARTIALLY_DEFINED` | Bullish in-range continuity and no-fragmentation by body color/doji are confirmed; terminal/reset candle geometry and independently evidenced bearish segmentation remain unresolved. |
| `STRUCTURAL_BODY_ANCHOR` | `PARTIALLY_DEFINED` | For a selected candidate-HL turn, use the lowest `Open` or `Close` among its bodies; unresolved terminal/reset membership and other turn edges prevent a complete selector. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Bullish in-range membership, color/doji continuity, new-High reset and prior-HL body-close invalidation now join the confirmed candidate and causal-validation rules; near-doji, gaps, remaining inclusivity, terminal membership, bearish symmetry and bootstrap remain non-reproducible. |
...
| Initial HL/LL/LH/HH candidate detection | `PARTIALLY_DEFINED` | Bullish candidate-HL window/deepest-low/base selection and price anchor are materially narrowed; exact-doji no-start, both same-candle origin-extreme precedence cases, bearish start/end, origin/causal validation, complex highest-candidate selection and selected-turn highest-body-edge coordinate are confirmed. Near-doji, gaps, exact inclusivity, turn membership, other class selection, pivots and lookbacks remain undefined. |
