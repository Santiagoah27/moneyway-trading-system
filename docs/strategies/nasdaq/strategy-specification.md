...
Target interaction and structural-break confirmation use different geometries. Step-1 and Step-4 structural breaks retain their audited body-close requirements. Target, liquidity and management interaction is confirmed by price touching the exact horizontal session-extreme level, including wick contact, without a close or tolerance.

The source does not establish a universal candle timeframe or event stream for observing target touches. It also does not establish precedence when one observable candle/frame both touches the target and supplies the final event needed to complete `NQ-M1-003`; OHLC alone may not expose intrabar order. Record these boundaries as `TARGET_TOUCH_OBSERVATION_TIMEFRAME_UNRESOLVED` and `SAME_OBSERVATION_ORDERING_UNRESOLVED`. Handling is also unresolved when a candidate was crossed before setup activation, both candidates coincide, one session level is unavailable or price is already beyond a candidate when the setup becomes active.
...
| 19. Break Even | `confirmed` conceptually + `human_validation_required` | Entry, London High, Asia High and observable price | On exact touch of the first encountered favorable session High, including wick contact, SL moves to entry | Not evaluated before entry; unresolved observation boundaries block automation | Yes | Observation timeframe, same-frame ordering, costs, universality and later management |
| 20. Take Profit | `confirmed` conceptually + `human_validation_required` | London High, Asia High and position state | Immediate target follows the first encountered relevant session High; one reviewed case exits completely on exact touch | No universal final-exit rule is established | Yes | First-target/final-TP relation, observation timeframe, edge cases, ratio and partials |
...
| 19. Break Even | `confirmed` conceptually + `human_validation_required` | Entry, London Low, Asia Low and observable price | On exact touch of the first encountered favorable session Low, including wick contact, SL moves to entry | Not evaluated before entry; unresolved observation boundaries block automation | Yes | Observation timeframe, same-frame ordering, costs, universality and later management |
| 20. Take Profit | `confirmed` conceptually + `human_validation_required` | London Low, Asia Low and position state | Immediate target follows the first encountered relevant session Low; one reviewed case exits completely on exact touch | No universal final-exit rule is established | Yes | First-target/final-TP relation, observation timeframe, edge cases, ratio and partials |
| 21. Trading-window end | `confirmed` | Clock in `America/Bogota` | Pre-entry workflow is outside `[08:30, 11:00)` at or after 11:00 | New or unfinished entry setup expires at 11:00 | No for timezone/DST | Post-entry management remains separately unresolved |
| 22. Risk controls | `confirmed` max trade risk + open controls | Entry, Stop Loss, sizing inputs | At most 1% risk after human validation | Missing sizing/control data blocks execution | Yes | Position size, daily limit, kill switch |
...
The newly confirmed management concept uses favorable session liquidity rather than treating an isolated post-entry swing as the canonical trigger. For a sell, the candidates are London Low and Asia Low; when price touches the first relevant candidate encountered downward, move Stop Loss to entry. For a buy, the candidates are London High and Asia High; when price touches the first relevant candidate encountered upward, move Stop Loss to entry. Exact contact with the horizontal wick extreme is sufficient, including wick contact, without body close, candle-close confirmation, tolerance or penetration buffer.

The relationship between this authoritative trigger and the earlier documented post-entry swing break is unresolved; the earlier swing rule must not replace the first-encountered session-liquidity concept automatically. The source says to move Stop Loss to entry/Break-Even; it does not define BE plus ticks, spread or commission adjustment. Universality, observation timeframe, same-observation ordering, commissions, spread, exact entry/cost basis, later management and whether this first target is also the final Take Profit remain open. The strategy concept is `confirmed`; a deterministic Break-Even algorithm is not yet fully defined.
...
Exact touch of the selected horizontal session-extreme level, including wick contact, is sufficient; no body close, candle close or tolerance is required. One reviewed example around Video 3 `13:25–13:40` supports complete exit when its selected level is touched. It does not establish that every first target is the final TP or define universal full-exit behavior. Observation timeframe, same-observation ordering, already-crossed/equal/unavailable/initially-beyond candidates, fixed ratio, partials, trailing and other final-target hierarchy remain unresolved.
