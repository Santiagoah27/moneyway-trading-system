...
Historical automation of `NQ-H4-001` therefore requires reconstruction using only information observable through each `AsOfUtc`. Before the causal close, the deepest low remains candidate; later observations cannot alter an earlier historical evaluation. The source defines the causal confirmation order and partially defines the bullish correction window, candidate-HL price selection and body anchoring, but not a complete detector: bootstrap of the prior reference level, doji/gap and candle-inclusivity boundaries, turn membership, candidate-LH selection, tie identity and coordinates for other structural-point classes remain unresolved.
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Bullish candidate-HL window and price selection are materially narrowed, but edge boundaries, segmentation, identity, bootstrap and candidate-LH selection remain non-reproducible. |
...
| Initial HL/LL/LH/HH candidate detection | `PARTIALLY_DEFINED` | Bullish candidate-HL window/deepest-low/base selection and price anchor are materially narrowed; doji/gap boundaries, turn membership, candidate-LH/other class selection, pivots and lookbacks remain undefined. |
