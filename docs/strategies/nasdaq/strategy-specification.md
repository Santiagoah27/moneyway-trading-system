...
- LH causal validation and the prior-LL correction origin are directly confirmed, not inferred. The mentor's verbal “same logic” statement supports only a `CANDIDATE` hypothesis that complex candidate-LH selection, highest-high replacement, correction-window construction and body anchoring mirror the bullish case; the reviewed material does not demonstrate those mechanics, so no symmetric deterministic algorithm is promoted.
...
- If several included bodies share that same lowest `Open`/`Close`, the structural price is still unambiguous when the strategy needs only that price; the identity of a particular candle/timestamp remains unresolved. Doji behavior before the first bearish candle, gaps, exact inclusivity of the HH/confirming candle, candidate-price scanning of the confirming candle, turn membership and the selected-candle identity remain unresolved.
...
The source now materially clarifies a retrospective bullish correction window: prior HH → first bearish correction candle → all correction observations before the causal confirming break → formally closed body above the prior HH. This lifecycle expression does not decide whether the causal breakout candle participates in candidate-price scanning. The window remains partially defined because doji/gap behavior and exact candle inclusivity are not supplied. It defines the lowest `Open`/`Close` price for a selected candidate-HL turn, including a price-preserving equal-coordinate tie, but not turn membership or candle identity. On the bearish side, only the prior-LL origin and causal close-below validation are directly demonstrated; mirrored complex selection and price anchoring remain candidate hypotheses. No wick-based anchor, fixed pivot, fixed lookback or symmetric selection formula is promoted to a confirmed rule.
...
| `BULLISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Retrospective window is prior HH → first bearish candle → observations before the causal break → confirming closed-body break; boundary edge cases and confirming-candle price-scan inclusivity remain unresolved. |
...
| `COMPLEX_CANDIDATE_LH_SELECTION` | `CANDIDATE` | Verbal same-logic guidance supports a symmetry hypothesis, but no reviewed complex bearish correction demonstrates highest-high replacement or discarding all intermediate highs. |
| `BEARISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Prior-LL origin and causal closed `Close < prior LL` end are confirmed; mirrored start, internal segmentation and complex-window mechanics are not directly demonstrated. |
| `CANDIDATE_LH_PRICE_SELECTION` | `CANDIDATE` | A mirrored highest-body price is only a symmetry hypothesis and is not confirmed. |
| `BEARISH_BODY_ANCHOR` | `CANDIDATE` | Mirroring the bullish body anchor is plausible from verbal guidance but lacks a direct demonstrated bearish case. |
| `CANDIDATE_LH_SELECTION` | `CANDIDATE` | Causal validation is confirmed, but complex selection mechanics remain an explicitly non-automatable symmetry hypothesis. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Bullish candidate-HL window/price and bearish causal validation/origin are materially narrowed; edge boundaries, identity, bootstrap and demonstrated complex candidate-LH selection remain non-reproducible. |
| `EXACT_STRUCTURAL_PRICE_COORDINATE` | `PARTIALLY_DEFINED` | The selected candidate-HL body anchor is defined, but turn membership and coordinates for candidate LH or other structural-point classes remain unresolved. |
...
| Candle/body used to place the line | `PARTIALLY_DEFINED` | A selected candidate-HL turn uses its lowest body `Open`/`Close`; turn membership, equal-coordinate identity and other structural-point coordinates are unresolved. |
...
| Initial HL/LL/LH/HH candidate detection | `PARTIALLY_DEFINED` | Bullish candidate-HL window/deepest-low/base selection and price anchor are materially narrowed; bearish origin/causal validation are confirmed, while complex candidate-LH selection/body anchor remain candidate hypotheses. Doji/gap boundaries, turn membership, other class selection, pivots and lookbacks remain undefined. |
| Reduction of a body zone to one decimal price | `UNRESOLVED` | No tolerance or coordinate rule is defined. |
