...
Historical automation of `NQ-H4-001` therefore requires reconstruction using only information observable through each `AsOfUtc`. Before the causal bullish or bearish close, the candidate remains unvalidated; later observations cannot alter an earlier historical evaluation. The source defines both correction starts and causal ends, resolves both origin-extreme precedence cases and their floor/ceiling updates, excludes the confirming candle from correction membership/scanning, confirms its validation role and single-candle `Open` origin coordinate, directly confirms the bearish prior-LL origin, complex candidate-LH selection, highest-high replacement and selected-turn highest-body-edge LH coordinate, and partially defines the bullish correction window, candidate-HL price selection and body anchoring. It does not define a complete detector: bootstrap of the prior reference level, doji/gap rules, boundary inclusivity, turn membership, tie identity and coordinates for other structural-point classes remain unresolved or partial.
...
| `BULLISH_CORRECTION_START` | `PARTIALLY_DEFINED` | The first bearish candle starts the correction after any same-candle higher `High` updates the origin ceiling; doji/gap behavior and exact HH-candle inclusivity are not defined. |
...
| `BULLISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Retrospective window includes the confirmed higher-High ceiling precedence before the first bearish close, then correction observations, impulsive expansion and confirming closed-body break; boundary edge cases and turn membership remain unresolved. |
...
| `BEARISH_CORRECTION_WINDOW` | `PARTIALLY_DEFINED` | Prior LL, cessation of new lows, first bullish-candle start, simultaneous lower-Low precedence/floor update, causal closed `Close < prior LL` end, highest-candidate replacement and confirming-candle exclusion are confirmed; doji/gap, exact LL-candle inclusivity and segmentation remain unresolved. |
...
| `CORRECTION_PHASE_BOUNDARIES` | `PARTIALLY_DEFINED` | Both directional starts, both same-candle origin-extreme precedence cases and both causal ends are confirmed; doji/gap handling, exact inclusivity and turn segmentation remain undefined. |
...
| `DOJI_BOUNDARY_BEHAVIOR` | `UNRESOLVED` | No rule says how doji/`Close == Open` candles interact with either confirmed correction-start rule. |
...
| `STRUCTURAL_CANDIDATE_DETECTION` | `PARTIALLY_DEFINED` | Confirming-candle roles, the single-candle `Open` origin coordinate and both same-candle origin-extreme precedence cases are defined alongside materially narrowed bullish/bearish candidate rules; doji/gap boundaries, exact inclusivity, segmentation and bootstrap remain non-reproducible. |
...
| Initial HL/LL/LH/HH candidate detection | `PARTIALLY_DEFINED` | Bullish candidate-HL window/deepest-low/base selection and price anchor are materially narrowed; both same-candle origin-extreme precedence cases, bearish start/end, origin/causal validation, complex highest-candidate selection and selected-turn highest-body-edge coordinate are confirmed. Doji/gap cases, exact inclusivity, turn membership, other class selection, pivots and lookbacks remain undefined. |
