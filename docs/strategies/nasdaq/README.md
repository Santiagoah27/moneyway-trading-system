...
On the relevant 1H or 4H structure, a candidate low becomes a validated HL only after the impulse originating from it produces a formally closed candle body above the prior HH; a candidate high becomes a validated LH only after the inverse body-close break below the prior LL. The same formally closed body requirement validates a new HH or LL, while wick-only penetration or a still-open candle is insufficient. This validation is causal: historical replay cannot use the candidate as validated before the confirming close. Initial candidate-turn detection and the exact body-zone price remain unresolved.
...
- Initial 1H/4H candidate-turn detection, retracement boundaries and exact structural body-zone coordinate; causal swing validation is confirmed.
...
- Initial HL/LL/LH/HH candidate detection, OHLC meaning of unmitigated, exact body-zone coordinate, and PDH/PDL ranking or ties within structural fallback.
