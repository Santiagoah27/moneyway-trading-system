...
- Bullish correction start/end and candidate-HL price selection are partially defined. Bearish first-bullish-candle start, causal `Close < prior LL` end, prior-LL origin, causal LH validation, complex candidate-LH selection, highest-high replacement and selected-turn highest-body-edge coordinate are confirmed. The bearish window remains partially defined only by doji/`Close == Open`, gap, simultaneous bullish-body/new-low, exact LL-candle inclusivity and turn-membership edge cases. Confirming-candle exclusion, single-candle `Open` structure and separate buy `Low`/sell `High` protection anchors remain confirmed; the actual Stop Loss offset remains unresolved.
