...
On the relevant 1H or 4H structure, a candidate low becomes a validated HL only after the impulse originating from it produces a formally closed candle body above the prior HH. On the bearish side, after a prior LL and cessation of new lows, the first bullish candle that begins upward displacement starts the correction. If that candle also prints a lower `Low`, the new extreme first updates the correction-origin floor and its bullish close then starts the correction from that floor; the new price extreme is not automatically a validated structural LL. Its candidate high remains provisional until a formally closed candle has `Close < prior LL`, when the correction ends and the new LL and preceding highest candidate high become validated from that causal `AsOfUtc`. Within the same recognized bearish correction, a higher candidate replaces a lower candidate and superseded intermediate highs do not survive as LH candidates. Wick-only penetration and a still-open candle are insufficient. Historical replay cannot use either candidate as validated before its confirming close. For an already selected LH turn, the structural coordinate is its highest candle-body edge: the maximum `Open` or `Close` among member candles, independent of candle color; a higher wick does not redefine it. Protection anchors remain separate: buy uses the relevant HL `Low`/lowest wick and sell uses the relevant LH `High`/highest wick, while the actual Stop Loss offset below/above that anchor remains unresolved. The causal break candle is excluded from correction membership and retrospective candidate scanning; it belongs to the impulsive expansion, and its close confirms the new HH/LL, ends the correction and validates the preceding candidate. In the single-expansive-candle case, that candle's `Open` supplies the structural HL/LH origin coordinate and its `Close` supplies causal validation without making it a correction candle or requiring an intrabar path. This case-specific rule does not replace multi-candle selected-turn body-edge rules. An exact doji with `Close == Open` is neutral and starts neither correction, although a same-candle new origin extreme still updates the floor or ceiling. The bearish window remains partial because near-doji thresholds, gaps, exact LL-candle inclusivity and segmentation remain unresolved. For bullish candidate HL, the prior HH remains the upper boundary; after HH production stops, the first bearish candle starts the correction. Once active, every in-range bearish, bullish or exact-doji candle remains in that same correction turn despite internal oscillation or consolidation, until a causal confirmation or terminal/reset event occurs; color alternation does not create parallel HL candidates. A strict `High > current correction-origin ceiling` resets the correction, updates the price ceiling and discards its candidate without automatically validating structural HH. A formally closed `Close < prior validated HL` invalidates the bullish structure; equality and wick-only penetration do not. The deepest low/base remains the candidate and replaces a shallower low; one- or two-candle pauses inside the confirming impulse do not create another HL. Within the selected turn, use the lowest body `Open`/`Close`, not an isolated wick. Equal lowest body coordinates preserve the price when only price is needed but do not choose candle identity. Near-doji/gap handling, remaining boundary inclusivity, terminal/reset candle geometry, bootstrap and bearish reset/invalidation symmetry remain unresolved.
...
- Exact doji (`Close == Open`) is neutral and starts neither correction; if it prints a new origin `High`/`Low`, the ceiling/floor updates while correction remains not started. Inside an already-active bullish correction, exact doji and alternating body colors remain in the same turn. Bullish in-range membership, strict new-High reset and prior-HL body-close invalidation are confirmed, while near-doji thresholds, gaps, remaining inclusivity, terminal/reset candle geometry, bootstrap and bearish reset/invalidation symmetry remain open. Bearish first-bullish-candle start, simultaneous lower-Low precedence/floor update, causal `Close < prior LL` end, prior-LL origin, causal LH validation, complex candidate-LH selection, highest-high replacement and selected-turn highest-body-edge coordinate are confirmed. In both directions, an updated correction-origin extreme remains distinct from a validated structural HH/LL. Confirming-candle exclusion, single-candle `Open` structure and separate buy `Low`/sell `High` protection anchors remain confirmed; the actual Stop Loss offset remains unresolved.
