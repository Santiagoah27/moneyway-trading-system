...
- Bullish correction start/end and candidate-HL price selection are partially defined; bearish prior-LL origin, causal LH validation, complex candidate-LH selection, highest-high replacement and selected-turn highest-body-edge coordinate are confirmed. Confirming-candle correction membership and scan exclusion are confirmed, as are its causal validation role, the single-candle origin/confirmation coexistence case and the single-candle `Open` structural coordinate. The bearish correction window remains partially defined; doji/gap handling, exact boundary inclusivity and turn membership remain non-deterministic. Buy `Low` and sell `High` protection anchors are separate from structural coordinates, and the actual Stop Loss offset remains unresolved.
