...
- Exact doji (`Close == Open`) is neutral and starts neither correction; if it prints a new origin `High`/`Low`, the ceiling/floor updates while correction remains not started. Bullish correction start/end and candidate-HL price selection remain partially defined; the simultaneous higher-High precedence/ceiling update is confirmed, while near-doji thresholds, gaps, exact HH-candle inclusivity and turn membership remain open. Bearish first-bullish-candle start, simultaneous lower-Low precedence/floor update, causal `Close < prior LL` end, prior-LL origin, causal LH validation, complex candidate-LH selection, highest-high replacement and selected-turn highest-body-edge coordinate are confirmed. In both directions, an updated correction-origin extreme remains distinct from a validated structural HH/LL. Confirming-candle exclusion, single-candle `Open` structure and separate buy `Low`/sell `High` protection anchors remain confirmed; the actual Stop Loss offset remains unresolved.
