...
On the relevant 1H or 4H structure, a candidate low becomes a validated HL only after the impulse originating from it produces a formally closed candle body above the prior HH; a candidate high becomes a validated LH only after the inverse body-close break below the prior LL. The same formally closed body requirement validates a new HH or LL, while wick-only penetration or a still-open candle is insufficient. This validation is causal: historical replay cannot use the candidate as validated before the confirming close. For candidate HL, preliminary/intermediate lows within a human-delimited correction are discarded: select the deepest low that is the base of the confirming impulse, and do not create a new HL from one- or two-candle pauses inside that impulse. Within the selected turn, use the lowest body `Open`/`Close`, not an isolated wick. Correction/turn boundaries and body ties remain unresolved, and candidate-LH selection is not inferred from this evidence.
...
- Algorithmic correction/turn boundaries, candidate-LH selection, equal-body tie handling and coordinates for structural points other than the partially defined candidate HL; causal swing validation remains confirmed.
