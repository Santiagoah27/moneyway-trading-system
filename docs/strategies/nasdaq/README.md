...
On the relevant 1H or 4H structure, a candidate low becomes a validated HL only after the impulse originating from it produces a formally closed candle body above the prior HH. Direct bearish evidence separately confirms that a retracement originates from the prior LL area and its candidate high remains provisional until a formally closed candle has `Close < prior LL`; only then are the new LL and preceding LH validated from that causal `AsOfUtc`. Wick-only penetration and a still-open candle are insufficient. Historical replay cannot use either candidate as validated before its confirming close. For bullish candidate HL, the prior HH remains the upper boundary; after HH production stops, the first bearish candle starts the correction, which stays active through deeper lows and internal zigzags until a formally closed `Close > prior HH`. The deepest low/base is the candidate and replaces a shallower low; one- or two-candle pauses inside the confirming impulse do not create another HL. Within the selected turn, use the lowest body `Open`/`Close`, not an isolated wick. Equal lowest body coordinates preserve the price when only price is needed but do not choose candle identity. Verbal same-logic guidance supports only a `candidate` symmetry hypothesis for complex candidate-LH selection, the mirrored bearish window and body anchor; no direct complex bearish demonstration makes those mechanics confirmed. Doji/gap handling, exact candle inclusivity and turn membership also remain unresolved.
...
- Bullish correction start/end and candidate-HL price selection are partially defined; bearish prior-LL origin and causal LH validation are confirmed. Complex candidate-LH selection, mirrored bearish window/body anchor, doji/gap handling, exact candle inclusivity, turn membership and structural-point identity are not confirmed deterministic rules.
