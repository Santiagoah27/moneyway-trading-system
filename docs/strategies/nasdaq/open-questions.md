...
| NQ-Q-H4-004 | ¿Cómo se corrige o reemplaza evidencia humana de bootstrap y se reinicializa H4 después de `StructureInvalidated`? | Valid derived structure and accepted initialization evidence do not expire at a session boundary. What happens after invalidation, how duplicate/conflicting anchors or reviewer corrections are handled, and the missing-input runtime result remain unspecified. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Human-reviewed examples of post-invalidation reconstruction, competing or corrected assertions and a missing-anchor replay frame |
...
- `H4_INITIAL_ANCHOR_LIFETIME = PARTIALLY_DEFINED`: a session boundary does not expire accepted bootstrap evidence, which remains auditable provenance even when a confirmed break normally rolls the active pair. `H4_INITIAL_ANCHOR_REINITIALIZATION`, `H4_POST_INVALIDATION_REINITIALIZATION`, `H4_INITIAL_ANCHOR_CONFLICT_POLICY` and `H4_MISSING_INITIAL_ANCHOR_RUNTIME_STATUS` remain `UNRESOLVED`. No reviewer-correction rule, duplicate precedence, post-invalidation reconstruction or missing-input status is copied from preparation completion.
