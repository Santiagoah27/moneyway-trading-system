...
The bullish candidate-confirmation plus origin-extension collision is resolved: one formally closed candle with `Close > prior structural HH` and `High > current correction-origin ceiling` validates the old candidate HL, excludes that candle from old-turn geometry, establishes the new HH-side extreme and rolls the active pair to new HH + validated HL. Generic reset does not discard the candidate in this compound event. The bearish `Close < prior LL` plus `Low < current correction-origin floor` collision is not explicitly resolved by the existing bearish reset/invalidation mirror. A confirming/extending candle with a corrective body has unresolved membership in the *next* correction turn; the old candidate is already validated, but no new turn is confirmed active from that fact. Confirmation plus opposite-point invalidation has no documented reference-order invariant that proves simultaneous strict closes impossible for all valid contexts, so its reachability and disposition remain open. Ordinary reset/start/await and reset/invalidation precedence remain separate. The paused active-turn composition primitive cannot yet emit one complete deterministic outcome for every supplied closed candle.
...
Broader structural states remain: `BULLISH_CORRECTION_START = CONFIRMED`; `BEARISH_CORRECTION_START = CONFIRMED`; `CANDIDATE_HL_SELECTION = PARTIALLY_DEFINED`; `CANDIDATE_LH_SELECTION = CONFIRMED` for highest-candidate identity within a recognized bearish correction; `BULLISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `BEARISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `TURN_MEMBERSHIP_SEGMENTATION = PARTIALLY_DEFINED`; `STRUCTURAL_CANDIDATE_DETECTION = PARTIALLY_DEFINED`; `NQ_H4_DIRECTION_BOOTSTRAP = PARTIALLY_DEFINED`. Pre-start waiting exclusion and non-confirming prior-HH/LL equality/wick-only inclusion remain confirmed. Bullish confirmation plus origin extension now validates the old candidate and rolls the active pair forward. Next-turn membership of its corrective-body confirming candle, the bearish confirmation-plus-extension mirror and confirmation-plus-invalidation reachability remain open alongside initial-anchor selection, historical scan termination, near-equal tolerance, near-doji treatment, other structural coordinates, mitigation, PDH/PDL ranking, TP hierarchy/full-exit behavior and executable Stop Loss offset.

Still unresolved: deterministic first-anchor selection and historical scan termination, near-equal structural-zone tolerance, near-doji/small-body threshold, other structural-point coordinates, mitigation semantics, executable Stop Loss offset, and the compound-event membership/precedence subcases above. Prior-HH/LL equality and wick-only geometry membership are resolved; the prior structural validation reference remains distinct from the correction-origin reset extreme. Exact equal-coordinate body identity is one price level with no candle tie-break. Exact `Close == Open` is neutral/no-start, and ordinary reset/invalidation terminal membership remains confirmed. The absence of a fixed lookback is confirmed; no invented pivot, threshold, tolerance or fixed-point initialization may replace the missing first-anchor rule. Replay must not relabel prior decisions using future evidence.
