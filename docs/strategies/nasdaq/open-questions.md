...
| NQ-Q-H4-002 | ¿Cómo se manejan dojis/`Close == Open`, gaps, candles contrarias que simultáneamente crean un nuevo extremo, la inclusividad exacta de los prior HH/LL boundaries y la membresía del turn? | Both directional correction starts/ends, confirming-candle exclusion and the single-candle `Open` origin coordinate are confirmed. The remaining edge semantics are not reproducible. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated bullish and bearish doji/gap, simultaneous contrary-body/new-extreme, boundary-inclusivity and turn-membership cases |
