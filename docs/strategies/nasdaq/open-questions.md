...
| NQ-Q-H4-005 | ¿Cómo se resuelven observaciones humanas duplicadas o corregidas del mismo vértice post-invalidation y qué resultado se devuelve si falta la evidencia? | [ADR 0007](../../decisions/0007-define-nasdaq-human-origin-vertex-replay-input.md) specifies the positive membership observation: exact H4 candle identities, invalidation-episode identity, causal time and provenance. Selection itself remains human-required; a single immutable value object is specifiable, but canonical transport, conflicting selections, correction/supersession and missing-evidence rule status are not. Given accepted members, geometry is deterministic. This is separate from initial raw-history bootstrap. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Human-reviewed competing and corrected assertions for one invalidation episode, a missing-evidence replay frame and a source-backed precedence or rejection policy |
...
- `H4_PROVISIONAL_VECTOR_ORIGIN_MEMBERSHIP = PARTIALLY_DEFINED`: the origin is the final turning cluster rather than the whole incoming leg; its defining extreme candle belongs, earlier clean directional progress is outside in the reviewed 5M example, and the invalidating candle begins the new impulse outside the vertex. Video 2 H4/1H review confirms visually grouped adjacent bodies may be close without exact equality; the backward multi-candle boundary and near-equal membership are `HUMAN_VALIDATION_REQUIRED`, while a numeric tolerance remains `UNRESOLVED`. Exact equal body prices retain one level without a candle tie-break; they do not select the cluster. A fixed count, candle-color, single-wick-extreme or numeric same-zone heuristic may not replace the reviewed members. The causal membership payload is now specified in ADR 0007; its orchestration policies remain open.
- [ADR 0007](../../decisions/0007-define-nasdaq-human-origin-vertex-replay-input.md) now specifies that causal vertex-membership observation's minimum payload: existing strategy/version/provider/symbol/time/provenance envelope, H4 timeframe, invalidating candle identity and exact selected member identities. The observation proves selection only; side comes from the verified transition, while geometry is calculated from the selected closed candles. It has no daily expiry and cannot revise earlier `AsOfUtc` results. `H4_HUMAN_ORIGIN_VERTEX_CONFLICT_POLICY`, `H4_HUMAN_ORIGIN_VERTEX_SUPERSESSION` and `H4_MISSING_HUMAN_VERTEX_RUNTIME_STATUS` remain `UNRESOLVED`; canonical transport and the full consumer remain unimplemented.
