...
|---|---|---|---|---|---|
| NQ-Q-H4-001 | ¿Qué regla reproducible selecciona el primer visible anchor estructural desde raw closed 4H candles? | The anchor concept, absence of fixed lookback, direction from the latest confirmed break and active-pair compression are confirmed, but an arbitrary raw series cannot establish its first structural reference automatically. | `human_validation_required` | `semi_automatic_backtesting` | Annotated starting histories showing included/excluded initial anchors and the exact historical scan termination boundary |
| NQ-Q-H4-002 | ¿Qué threshold, si alguno, distingue un near-doji/small body y cómo se maneja la inclusividad restante? | Exact `Close == Open` is neutral; mirrored bullish/bearish in-range membership, reset, invalidation, collision precedence and opening-gap behavior are confirmed. Near-doji and remaining inclusivity remain non-reproducible. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Numeric near-doji evidence plus annotated remaining boundary-inclusivity cases |
...
- No fixed candle/day/session count, time window, scan count, pivot period or numeric lookback exists for 4H initialization: `H4_FIXED_LOOKBACK_RULE = CONFIRMED_NONE`. This does not define an unbounded deterministic scan or its stopping boundary.
- The mentor's initial visible High/Low that originated the current directional leg is a confirmed concept, while choosing that first anchor from arbitrary raw candles remains `human_validation_required`. Viewport, zoom, screen width and arbitrary loaded history cannot become hidden engine inputs.
- Once structural references exist, the latest strict formally closed body break above/below an established upper/lower level supplies inherited bullish/bearish direction. Equality, wick-only penetration and current candle colors alone do not.
- After that break, operational history compresses to the latest HH+HL or LL+LH active pair under existing causal candidate rules; older structure remains auditable. This does not introduce a pivot algorithm.
- The instruction to find the latest broken structural level is circular at initialization because the compared historical structural levels must already have been established. `H4_RAW_CANDLE_INITIALIZATION = HUMAN_VALIDATION_REQUIRED` and full deterministic bootstrap remains blocked by initial-anchor selection.
...
- The bearish correction start, exact-doji no-start behavior, in-range turn membership, strict new-Low reset, same-candle bullish restart, prior-LH body-close invalidation, invalidation-candle exclusion and invalidation-dominant collision are confirmed. Near-doji treatment, remaining LL-boundary inclusivity and broader correction/impulse segmentation remain unresolved. The causal break candle is confirmed excluded from correction membership/scanning and belongs to the impulsive leg. Within an already selected LH turn, the structural coordinate and bearish body anchor are confirmed as the highest `Open`/`Close` body edge among member candles.
...
- Exact-doji behavior and mirrored bullish/bearish terminal/reset membership are confirmed. Near-doji thresholds and remaining prior-HH/LL boundary inclusivity remain unresolved. Exact equal body coordinates define one structural price level without a candle/timestamp tie-break; near-equal merging tolerance remains unresolved.
...
Broader structural states remain: `BULLISH_CORRECTION_START = CONFIRMED`; `BEARISH_CORRECTION_START = CONFIRMED`; `CANDIDATE_HL_SELECTION = PARTIALLY_DEFINED`; `CANDIDATE_LH_SELECTION = CONFIRMED` for highest-candidate identity within a recognized bearish correction; `BULLISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `BEARISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `TURN_MEMBERSHIP_SEGMENTATION = PARTIALLY_DEFINED`; `STRUCTURAL_CANDIDATE_DETECTION = PARTIALLY_DEFINED`; `NQ_H4_DIRECTION_BOOTSTRAP = PARTIALLY_DEFINED`. `PRE_START_WAITING_CANDLE_MEMBERSHIP = CONFIRMED_EXCLUDED_FROM_CORRECTION` now covers every waiting candle, including after `ResetAndAwaitCorrectionStart`; `DISTINCT_EXTREME_CANDLE_MEMBERSHIP` and `PRE_START_EXACT_DOJI_AT_EXTREME` remain `CONFIRMED_EXCLUDED_FROM_CORRECTION`. Exact body direction and the full waiting boundary justify confirming the bullish start without defining a near-doji threshold. Initial-anchor selection, historical scan termination, near-equal-zone tolerance, near-doji treatment beyond exact starts, remaining inclusivity, remaining structural coordinates, mitigation, PDH/PDL ranking or ties, TP hierarchy/full-exit behavior and the executable Stop Loss offset remain open. Active-turn membership, same-candle reset-and-start, body-anchor, protection-anchor and future-data states remain unchanged.

Still unresolved: deterministic first-anchor selection and historical scan termination, near-equal structural-zone tolerance, near-doji/small-body threshold, remaining prior-HH/LL boundary inclusivity, other structural-point coordinates, mitigation semantics and the executable Stop Loss offset beyond the confirmed protection anchor. Exact equal-coordinate body identity is closed as one price level with no candle tie-break. Exact `Close == Open` is neutral/no-start; both directions' in-range and terminal membership, causal ends and same-candle origin-extreme precedence are no longer open. The absence of a fixed lookback is confirmed; no fractal, ZigZag, N-left/N-right, ATR, percentage-swing, candle-count threshold, tolerance or fixed-point initialization may replace the missing first-anchor rule. Replay must not relabel the doji-time no-start decision, terminal membership or candidate HL/LH confirmation using future evidence; origin-extreme updates remain separate from structural HH/LL validation.
...
| Question ID | Question | Why it matters | Current status | Blocking level | Proposed evidence needed |
|---|---|---|---|---|---|
| NQ-Q-LQ-001 | ¿Cómo se manejan near-dojis y remaining boundary inclusivity dentro de las correction windows; qué significa no mitigado y cómo se ordenan PDH/PDL frente a 1H/4H? | Exact doji no-start, opening-gap no-special-role behavior, mirrored terminal membership, confirming-candle correction membership/scan exclusion, its causal validation role and the single-candle `Open` origin coordinate are confirmed. Remaining window edges, mitigation and cross-class ranking remain unresolved or partial. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Numeric near-doji evidence, annotated remaining boundary cases, other structural classes, mitigation boundaries, PDH/PDL coexistence and ranking ties |
