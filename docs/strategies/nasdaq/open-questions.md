...
| NQ-Q-H4-002 | ¿Qué threshold, si alguno, distingue un near-doji/small body; cómo se manejan gaps, la inclusividad restante y la simetría bearish de candles terminales/reset? | Exact `Close == Open` is neutral; bullish in-range membership and bullish reset/invalidation terminal membership are confirmed. Near-doji, gaps, remaining inclusivity and bearish terminal symmetry remain non-reproducible. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Numeric near-doji evidence plus annotated gap and remaining boundary-inclusivity cases; separate direct evidence for bearish reset/invalidation symmetry |
...
- Exact-doji behavior and bullish terminal/reset membership are confirmed. Near-doji thresholds, gaps, remaining prior-HH/LL boundary inclusivity and bearish reset/invalidation symmetry remain unresolved. Equal body coordinates do not make the selected price ambiguous when only price is needed, but candle/timestamp identity remains unresolved.
...
Current bullish terminal boundaries: `BULLISH_NEW_HIGH_RESET_CANDLE_DUAL_ROLE = CONFIRMED`; `BULLISH_RESET_BEARISH_CANDLE_NEW_TURN_MEMBERSHIP = CONFIRMED_INCLUDED`; `BULLISH_RESET_NEUTRAL_CANDLE_CORRECTION_START = CONFIRMED_NO_START`; `BULLISH_RESET_BULLISH_CANDLE_CORRECTION_START = CONFIRMED_NO_START`; `BULLISH_INVALIDATION_CANDLE_PREVIOUS_TURN_MEMBERSHIP = CONFIRMED_EXCLUDED`; `BULLISH_INVALIDATION_CANDLE_PREVIOUS_CANDIDATE_GEOMETRY = CONFIRMED_EXCLUDED`; `BULLISH_INVALIDATION_CANDLE_NEW_BEARISH_IMPULSE_ROLE = CONFIRMED`; `BEARISH_RESET_INVALIDATION_SYMMETRY = UNRESOLVED`.
...
Broader structural states remain: `CANDIDATE_HL_SELECTION = PARTIALLY_DEFINED`; `BULLISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `TURN_MEMBERSHIP_SEGMENTATION = PARTIALLY_DEFINED`; `STRUCTURAL_CANDIDATE_DETECTION = PARTIALLY_DEFINED`. Collision precedence narrows these boundaries but does not resolve near-doji thresholds, gaps, remaining inclusivity, bootstrap/lookback, bearish-side symmetry, remaining structural coordinates or identity, mitigation, PDH/PDL ranking or ties, TP hierarchy/full-exit behavior, or the executable Stop Loss offset. All previously audited validation, exact-doji, origin-extreme precedence, body-anchor, protection-anchor and future-data states remain unchanged.

Still unresolved: a reproducible viewport for the visual bootstrap, near-doji/small-body threshold, gap handling, remaining prior-HH/LL boundary inclusivity, independently evidenced bearish reset/invalidation symmetry, equal-coordinate body identity, other structural-point coordinates, mitigation semantics and the executable Stop Loss offset beyond the confirmed protection anchor. Exact `Close == Open` is neutral/no-start; bullish in-range and terminal membership, both directional causal ends and both demonstrated same-candle origin-extreme precedence cases are no longer open. No fixed-lookback, fractal, ZigZag, N-left/N-right, ATR, percentage-swing, candle-count threshold, tolerance or fixed-point initialization is supplied. Replay must not relabel the doji-time no-start decision, terminal membership or candidate HL/LH confirmation using future evidence; origin-extreme updates remain separate from structural HH/LL validation.
...
| Question ID | Question | Why it matters | Current status | Blocking level | Proposed evidence needed |
|---|---|---|---|---|---|
| NQ-Q-LQ-001 | ¿Cómo se manejan near-dojis, gaps, remaining boundary inclusivity y bearish turn membership dentro de las correction windows; qué significa no mitigado y cómo se ordenan PDH/PDL frente a 1H/4H? | Exact doji no-start, bullish terminal membership, confirming-candle correction membership/scan exclusion, its causal validation role and the single-candle `Open` origin coordinate are confirmed. Remaining window edges, bearish symmetry, mitigation and cross-class ranking remain unresolved or partial. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Numeric near-doji evidence, annotated gap/boundary and bearish terminal cases, other structural classes, mitigation boundaries, PDH/PDL coexistence and ranking ties |
