...
| NQ-Q-H4-002 | ¿Qué threshold, si alguno, distingue un near-doji/small body; cómo se manejan gaps, la inclusividad exacta de los prior HH/LL boundaries y la membresía de candles terminales/reset? | Exact `Close == Open` is neutral; bullish in-range candles of any body direction remain in one active turn, strict new-High resets it and closed-body loss of the prior HL invalidates it. Near-doji, gaps and terminal/reset geometry membership remain non-reproducible. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Numeric near-doji evidence plus annotated gap, boundary-inclusivity and bullish terminal/reset membership cases; separate direct evidence for bearish symmetry |
| NQ-Q-H4-003 | ¿Cuándo runtime necesita identidad de candle/timestamp para el body anchor y cuáles son las coordenadas para structural points distintos de los candidate HL/LH? | Equal body coordinates preserve one candidate price when only price is needed, but tie identity, terminal/reset candle geometry and other-point coordinates remain undefined. For a selected LH turn, the highest `Open`/`Close` body edge is confirmed; the separate sell SL highest-wick anchor does not alter it. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated equal-coordinate identity cases, terminal/reset turn edges and direct coordinates for other structural-point classes |
...
- During the active bullish correction, `High > current correction-origin ceiling` resets the correction, updates the ceiling and discards its current candidate HL. That higher price extreme is not automatically a validated structural HH.
- A formally closed `Close < prior validated HL` invalidates the bullish structure and ends pursuit of the same candidate HL. Equality and wick-only `Low < prior HL` with `Close >= prior HL` do not invalidate under this rule. Membership of the reset/invalidation candle in candidate geometry remains unresolved.
...
- The bearish correction start, exact-doji no-start behavior, bullish-candle/new-low precedence and end are confirmed, but near-doji, gaps, exact LL-candle inclusivity and correction segmentation remain unresolved. The causal break candle is confirmed excluded from correction membership/scanning and belongs to the impulsive leg. Within an already selected LH turn, the structural coordinate and bearish body anchor are confirmed as the highest `Open`/`Close` body edge among member candles.
...
- Exact-doji behavior is confirmed. Near-doji thresholds, gaps, exact prior-HH/LL boundary inclusivity and terminal/reset candle membership remain unresolved. Equal body coordinates do not make the selected price ambiguous when only price is needed, but candle/timestamp identity remains unresolved.
...
Current boundaries: `STRUCTURAL_VALIDATION_RULE = CONFIRMED`; `LH_CAUSAL_VALIDATION = CONFIRMED`; `BEARISH_CORRECTION_ORIGIN = CONFIRMED`; `BEARISH_CORRECTION_START = CONFIRMED`; `BEARISH_CORRECTION_END = CONFIRMED`; `BULLISH_CANDLE_WITH_NEW_LOW_PRECEDENCE = CONFIRMED`; `CORRECTION_ORIGIN_FLOOR_UPDATE = CONFIRMED`; `CORRECTION_START_AFTER_UPDATED_FLOOR = CONFIRMED`; `CORRECTION_ORIGIN_EXTREME_VS_STRUCTURAL_LL = CONFIRMED`; `BEARISH_CANDLE_WITH_NEW_HIGH_PRECEDENCE = CONFIRMED`; `CORRECTION_ORIGIN_CEILING_UPDATE = CONFIRMED`; `CORRECTION_START_AFTER_UPDATED_CEILING = CONFIRMED`; `CORRECTION_ORIGIN_EXTREME_VS_STRUCTURAL_HH = CONFIRMED`; `EXACT_DOJI_CORRECTION_START = CONFIRMED_NO_START`; `DOJI_DIRECTIONAL_ROLE = CONFIRMED_NEUTRAL`; `DOJI_WITH_NEW_HIGH_PRECEDENCE = CONFIRMED`; `DOJI_WITH_NEW_LOW_PRECEDENCE = CONFIRMED`; `NEAR_DOJI_THRESHOLD = UNRESOLVED`; `CANDIDATE_HL_SELECTION = PARTIALLY_DEFINED`; `BULLISH_CORRECTION_START = PARTIALLY_DEFINED`; `BULLISH_CORRECTION_END = CONFIRMED`; `BULLISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `BULLISH_CORRECTION_TURN_MEMBERSHIP = CONFIRMED`; `BULLISH_INTERNAL_COLOR_ALTERNATION_MEMBERSHIP = CONFIRMED`; `BULLISH_DOJI_INSIDE_ACTIVE_CORRECTION = CONFIRMED`; `BULLISH_CORRECTION_NEW_HIGH_RESET = CONFIRMED`; `BULLISH_CANDIDATE_RESET_ON_NEW_HIGH = CONFIRMED`; `BULLISH_PRIOR_HL_BODY_CLOSE_INVALIDATION = CONFIRMED`; `BULLISH_PRIOR_HL_WICK_ONLY_INVALIDATION = CONFIRMED_NO_INVALIDATION`; `BULLISH_TERMINAL_RESET_CANDLE_MEMBERSHIP = UNRESOLVED`; `BEARISH_RESET_INVALIDATION_SYMMETRY = UNRESOLVED`; `CANDIDATE_HL_PRICE_SELECTION = PARTIALLY_DEFINED`; `CANDIDATE_HL_IDENTITY_SELECTION = UNRESOLVED`; `DOJI_BOUNDARY_BEHAVIOR = PARTIALLY_DEFINED`; `COMPLEX_CANDIDATE_LH_SELECTION = CONFIRMED`; `HIGHEST_HIGH_REPLACEMENT = CONFIRMED`; `BEARISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `CANDIDATE_LH_STRUCTURAL_PRICE = CONFIRMED`; `BEARISH_STRUCTURAL_BODY_ANCHOR = CONFIRMED`; `SELL_LH_SL_ANCHOR = CONFIRMED`; `STRUCTURAL_IDENTITY_VS_PRICE_SEPARATION = CONFIRMED`; `CONFIRMING_CANDLE_CORRECTION_MEMBERSHIP = CONFIRMED_EXCLUDED`; `CONFIRMING_CANDLE_VALIDATION_ROLE = CONFIRMED`; `IMPULSE_LEG_MEMBERSHIP = PARTIALLY_DEFINED`; `SINGLE_CANDLE_BREAK_ORIGIN_CASE = CONFIRMED`; `CONFIRMING_CANDLE_SCAN_INCLUSIVITY = CONFIRMED_EXCLUDED`; `SINGLE_CANDLE_ORIGIN_PRICE_COORDINATE = CONFIRMED`; `SINGLE_CANDLE_HL_STRUCTURAL_PRICE = CONFIRMED`; `SINGLE_CANDLE_LH_STRUCTURAL_PRICE = CONFIRMED`; `BUY_HL_PROTECTION_ANCHOR = CONFIRMED`; `SELL_LH_PROTECTION_ANCHOR = CONFIRMED`; `ACTUAL_SL_OFFSET_FROM_ANCHOR = UNRESOLVED`; `STRUCTURE_VS_PROTECTION_SEPARATION = CONFIRMED`; `CORRECTION_PHASE_BOUNDARIES = PARTIALLY_DEFINED`; `TURN_MEMBERSHIP_SEGMENTATION = PARTIALLY_DEFINED`; `STRUCTURAL_BODY_ANCHOR = PARTIALLY_DEFINED`; `BODY_ANCHOR_TIE_BREAKING = UNRESOLVED`; `STRUCTURAL_CANDIDATE_DETECTION = PARTIALLY_DEFINED`.

Still unresolved: a reproducible viewport for the visual bootstrap, near-doji/small-body threshold, gap handling, exact prior-HH/LL boundary inclusivity, terminal/reset candle geometry, independently evidenced bearish reset/invalidation symmetry, equal-coordinate body identity, other structural-point coordinates, mitigation semantics and the executable Stop Loss offset beyond the confirmed protection anchor. Exact `Close == Open` is neutral/no-start; bullish in-range turn membership, both directional causal ends and both demonstrated same-candle origin-extreme precedence cases are no longer open. No fixed-lookback, fractal, ZigZag, N-left/N-right, ATR, percentage-swing, candle-count threshold, tolerance or fixed-point initialization is supplied. Replay must not relabel the doji-time no-start decision or label candidate HL/LH as confirmed using future evidence; origin-extreme updates remain separate from directional correction start.
...
| Question ID | Question | Why it matters | Current status | Blocking level | Proposed evidence needed |
|---|---|---|---|---|---|
| NQ-Q-LQ-001 | ¿Cómo se manejan near-dojis, gaps, boundary inclusivity y turn membership dentro de las correction windows; qué significa no mitigado y cómo se ordenan PDH/PDL frente a 1H/4H? | Exact doji no-start, confirming-candle correction membership/scan exclusion, its causal validation role and the single-candle `Open` origin coordinate are confirmed. Remaining window edges, mitigation and cross-class ranking remain unresolved or partial. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Numeric near-doji evidence, annotated gap/boundary cases, other structural classes, mitigation boundaries, PDH/PDL coexistence and ranking ties |
