...
| NQ-Q-H4-002 | ¿Cómo se manejan dojis/gaps, la inclusividad del HH/candle confirming y la membresía exacta del turn dentro de la bullish correction window; existe evidencia directa para seleccionar candidate LH? | Prior HH → first bearish candle → causal closed break materially narrows the bullish window, but its edge boundaries, turn segmentation and inverse candidate-LH selection are not reproducible. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated doji/gap and inclusivity cases, turn membership, excluded intermediate lows/impulse pauses and direct candidate-LH evidence |
...
- The inverse causal validation of an already-supplied candidate LH is confirmed, but candidate-LH selection itself is not confirmed by the newly reviewed evidence and is not inferred symmetrically.
...
Current boundaries: `STRUCTURAL_VALIDATION_RULE = CONFIRMED`; `CANDIDATE_HL_SELECTION = PARTIALLY_DEFINED`; `BULLISH_CORRECTION_START = PARTIALLY_DEFINED`; `BULLISH_CORRECTION_END = CONFIRMED`; `BULLISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `CANDIDATE_HL_PRICE_SELECTION = PARTIALLY_DEFINED`; `CANDIDATE_HL_IDENTITY_SELECTION = UNRESOLVED`; `DOJI_BOUNDARY_BEHAVIOR = UNRESOLVED`; `CANDIDATE_LH_SELECTION = UNRESOLVED`; `CORRECTION_PHASE_BOUNDARIES = PARTIALLY_DEFINED`; `STRUCTURAL_BODY_ANCHOR = PARTIALLY_DEFINED`; `BODY_ANCHOR_TIE_BREAKING = UNRESOLVED`; `STRUCTURAL_CANDIDATE_DETECTION = PARTIALLY_DEFINED`.

Still unresolved: a reproducible viewport for the visual bootstrap, doji/gap handling, exact HH/confirming-candle inclusivity, turn membership, candidate-LH selection, equal-coordinate body identity, other structural-point coordinates and mitigation semantics. No fixed-lookback, fractal, ZigZag, N-left/N-right, ATR, percentage-swing, candle-count threshold, tolerance or fixed-point initialization is supplied. Replay must not label the selected candidate HL as confirmed before the subsequent break closes; the confirming close changes status only from its own observable `AsOfUtc` onward.
...
| Question ID | Question | Why it matters | Current status | Blocking level | Proposed evidence needed |
|---|---|---|---|---|---|
| NQ-Q-LQ-001 | ¿Cómo se manejan dojis/gaps, candle inclusivity y turn membership dentro de la bullish correction window; cómo se seleccionan candidate LH y otros structural points, qué significa no mitigado y cómo se ordenan PDH/PDL frente a 1H/4H? | Bullish candidate-HL window/deepest-low/base and its lowest-body price are partially defined, while edge boundaries, inverse/other candidate selection, mitigation and cross-class ranking are not. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated doji/gap/inclusivity and turn cases, candidate LH/other classes, body-anchor identities, mitigation boundary cases, PDH/PDL coexistence and ranking ties |
