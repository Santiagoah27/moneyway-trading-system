...
| NQ-Q-H4-002 | ¿Cómo se manejan dojis/gaps, la inclusividad del HH/candle confirming y la membresía exacta del turn dentro de la bullish correction window; qué caso demuestra directamente una complex candidate-LH selection? | Prior HH → first bearish candle → causal closed break materially narrows the bullish window. Prior-LL origin and causal LH validation are also directly confirmed, but bullish edge boundaries and mirrored complex candidate-LH mechanics are not reproducible. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated bullish doji/gap and inclusivity cases, turn membership, plus a demonstrated complex bearish correction with included/excluded highs |
| NQ-Q-H4-003 | ¿Cuándo runtime necesita identidad de candle/timestamp para el body anchor y cuál es la coordenada para structural points distintos del candidate HL? | Equal lowest body coordinates preserve a single candidate-HL price when only price is needed, but tie identity, turn membership and other structural-point coordinates remain undefined. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated multi-candle turns, equal-coordinate identity cases and direct coordinates for candidate LH/other point classes |
...
- The mentor's verbal same-logic statement supports a `CANDIDATE` symmetry hypothesis for complex candidate-LH selection, highest-high replacement, a mirrored bearish correction window and body anchor. None is promoted to confirmed because the reviewed material does not demonstrate a complex bearish correction.
...
| Question ID | Question | Why it matters | Current status | Blocking level | Proposed evidence needed |
|---|---|---|---|---|---|
| NQ-Q-LQ-001 | ¿Cómo se manejan dojis/gaps, candle inclusivity y turn membership dentro de la bullish correction window; qué direct complex bearish case define candidate LH/body anchor; qué significa no mitigado y cómo se ordenan PDH/PDL frente a 1H/4H? | Bullish candidate-HL construction is partially defined and bearish causal validation/origin are confirmed, while mirrored complex candidate-LH construction is only a symmetry hypothesis; mitigation and cross-class ranking are also unresolved. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated bullish edge cases, a demonstrated complex bearish correction/body anchor, other structural classes, mitigation boundary cases, PDH/PDL coexistence and ranking ties |
