...
| NQ-Q-H4-006 | ¿Qué observación exacta significa que el precio excede el Candidate LH/HL provisional antes de la segunda ruptura? | Human review confirms that an exceeded provisional candidate is discarded and the correction resumes/rebuilds, but does not specify whether to compare a wick, `ProtectionAnchor`, body edge, `StructuralPrice` or `Close`; strictness, equality and tolerance also remain open. The first closed opposite-body candle can form a provisional candidate without resolving this later transition. | `UNRESOLVED` | `semi_automatic_backtesting` | Annotated closed H4 examples that identify the compared price/coordinate, equality behavior and causal candle for candidate discard before the frozen-terminal second break |
