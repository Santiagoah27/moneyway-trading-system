...
| NQ-Q-H4-005 | ¿Qué coordenadas y membership delimitan el vector provisional tras `StructureInvalidated`? | Opposite intent and the `H -> L` or `L -> H` transition are confirmed, but the origin point, provisional terminal body-versus-wick comparison level and invalidating-candle inclusion in those extrema are not source-defined. Existing calculators require supplied references and a valid side. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated invalidation-to-second-break sequences giving exact origin and terminal comparison prices, extreme updates and candidate-turn boundary for both directions |
...
- `H4_PROVISIONAL_VECTOR_COORDINATES = UNRESOLVED`: the departure origin is not selected by any existing primitive, and neither the provisional terminal body-versus-wick comparison level nor the invalidating candle's exact origin/terminal membership is defined. `CorrectionOriginExtremeCalculator` updates a supplied wick extreme but does not initialize the vector or define a structural comparison coordinate. The confirmed sequence is not yet a complete deterministic reconstruction contract.
