...
| NQ-Q-H4-005 | ¿Qué candle(s) delimitan el vértice de origen H/L del impulso opuesto tras `StructureInvalidated`? | Its body-edge StructuralPrice and wick ProtectionAnchor formulas are confirmed once the origin vertex is selected, but the departure source/candle set is not determined from the prior active pair, correction-origin extreme or invalidation candle by an existing selector. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated invalidation paths identifying the exact departure-origin event and included candles for both directions |
| NQ-Q-H4-006 | ¿Qué ocurre cuando la primera vela de cuerpo correctivo también extiende el extremo del impulso opuesto provisional? | Existing valid-side semantics can update the extreme and start correction in one candle, but direct post-invalidation evidence does not decide whether that rule applies here or the candle stays outside the new correction. The supplied `Open = 101`, `Close = 100.5` example is bearish, not a bullish corrective body for an emerging bearish impulse. | `UNRESOLVED` | `semi_automatic_backtesting` | Direct human-reviewed bullish-body/new-Low and bearish-body/new-High post-invalidation examples with turn membership and frozen terminal references |
...
- `H4_PROVISIONAL_VECTOR_COORDINATES = PARTIALLY_DEFINED`: source-backed body-edge `StructuralPrice` and separate wick `ProtectionAnchor` apply to selected provisional high/low vertices; the included bearish impulse's minimum body edge is provisional L's later break reference, with minimum `Low` as protection, and the bullish mirror uses maxima. Different candles can supply body and wick extrema; equal body coordinates share one level. The invalidating candle is the first new-impulse member and can be its terminal extreme when correction begins immediately. Its terminal is not frozen if later impulse candles extend it; once correction starts, the completed impulse terminal freezes and correction candles form candidate LH/HL.
- `H4_PROVISIONAL_VECTOR_ORIGIN_MEMBERSHIP` remains `UNRESOLVED`: the exact departure-origin vertex/candle set is not selected by existing primitives. Separately, `H4_POST_INVALIDATION_CORRECTION_START_EXTREME_COLLISION = UNRESOLVED`: generic valid-side same-candle update-and-start behavior cannot be transferred silently to this provisional context. A corrective-body/new-extreme candle's membership and terminal freeze need direct evidence. The supplied `Open = 101`, `Close = 100.5` example has a bearish body, so it does not decide the proposed emerging-bearish collision.
