...
Broader structural states remain: `BULLISH_CORRECTION_START = PARTIALLY_DEFINED`; `BEARISH_CORRECTION_START = CONFIRMED`; `CANDIDATE_HL_SELECTION = PARTIALLY_DEFINED`; `CANDIDATE_LH_SELECTION = CONFIRMED` for highest-candidate identity within a recognized bearish correction; `BULLISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `BEARISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `TURN_MEMBERSHIP_SEGMENTATION = PARTIALLY_DEFINED`; `STRUCTURAL_CANDIDATE_DETECTION = PARTIALLY_DEFINED`; `NQ_H4_DIRECTION_BOOTSTRAP = PARTIALLY_DEFINED`. `DISTINCT_EXTREME_CANDLE_MEMBERSHIP = CONFIRMED_EXCLUDED_FROM_CORRECTION` and `PRE_START_EXACT_DOJI_AT_EXTREME = CONFIRMED_EXCLUDED_FROM_CORRECTION` close the distinct pre-start boundary in both directions. Exact equal-price identity is also confirmed. Initial-anchor selection, historical scan termination, near-equal-zone tolerance, near-doji thresholds, remaining inclusivity, remaining structural coordinates, mitigation, PDH/PDL ranking or ties, TP hierarchy/full-exit behavior and the executable Stop Loss offset remain open. All previously audited validation, active-turn membership, same-candle reset-and-start, body-anchor, protection-anchor and future-data states remain unchanged.
