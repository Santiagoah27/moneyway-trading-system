...
| NQ-Q-H4-007 | ¿Qué precedencia y pertenencia por componente aplica si una misma vela H4 migra el extremo provisional y cierra más allá del terminal congelado? | Both bearish `High > Candidate ProtectionAnchor` with `Close < frozen provisional L` and bullish `Low < Candidate ProtectionAnchor` with `Close > frozen provisional H` are valid OHLC combinations. The strict wick migration and strict body-close break are individually defined, but the source does not decide their same-candle precedence or definitive body/wick membership. | `UNRESOLVED` | `semi_automatic_backtesting` | Human-reviewed same-candle examples for both directions identifying whether confirmation or migration prevails and which components of that candle enter the final candidate |

Confirmed or materially narrowed by the new human source review:

- `NQ-Q-H4-006` is resolved by direct review of Video 2 approximately `12:45–13:45` and Video 3 approximately `07:40–07:46`: provisional HL migrates only on `Low < Candidate ProtectionAnchor`; provisional LH migrates only on `High > Candidate ProtectionAnchor`. Equality does not migrate, and no body, Close or tolerance comparison replaces the strict wick rule. Earlier provisional states remain causal replay evidence; only the separate same-candle migration/second-break collision in `NQ-Q-H4-007` remains open.
