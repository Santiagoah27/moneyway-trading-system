...
| NQ-Q-H4-002 | ¿Cómo se manejan dojis/gaps, la inclusividad exacta de los prior HH/LL boundaries, la membresía del turn y la coordenada del impulse origin cuando una sola candle también confirma el break? | The confirming candle is excluded from correction membership/scanning and belongs to the impulsive leg; it may contain origin evidence in the single-candle case. The remaining edge and coordinate semantics are not reproducible. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated bullish and bearish doji/gap, boundary-inclusivity, turn-membership and single-candle-origin coordinate cases |
...
- In the single-expansive-candle case, the same candle may contain impulse-origin evidence and the confirming close without becoming an ordinary correction member. The exact origin-price coordinate remains unresolved, and no intrabar chronology or synthetic OHLC path is inferred.
...
| Question ID | Question | Why it matters | Current status | Blocking level | Proposed evidence needed |
|---|---|---|---|---|---|
| NQ-Q-LQ-001 | ¿Cómo se manejan dojis/gaps, boundary inclusivity y turn membership dentro de las correction windows; qué coordenada representa el origin en el single-candle break, qué significa no mitigado y cómo se ordenan PDH/PDL frente a 1H/4H? | Confirming-candle correction membership/scan exclusion and its causal validation role are confirmed. Window edges, the single-candle origin coordinate, mitigation and cross-class ranking remain unresolved or partial. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated bullish/bearish edge and single-candle-origin cases, other structural classes, mitigation boundaries, PDH/PDL coexistence and ranking ties |
