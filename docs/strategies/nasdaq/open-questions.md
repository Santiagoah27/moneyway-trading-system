...
| NQ-Q-SC-003 | ¿Cómo observa el replay que Steps 1 y 2 se completaron para el mismo día antes de 08:30? | [ADR 0006](../../decisions/0006-define-nasdaq-preparation-replay-input-contract.md) specifies one immutable, source-backed positive completion assertion for session `D`, with a causal UTC timestamp and `AsOfUtc` isolation. `StrategyReplayContext` does not yet carry it; human-dependent `NQ-H4-001` and `NQ-LIQ-002` cannot be inferred from candles. | `confirmed` | `semi_automatic_backtesting` | Implement canonical replay input/context transport before the evaluator; no producer, storage or UI mechanism is selected here |
