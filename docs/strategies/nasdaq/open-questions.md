...
| NQ-Q-H4-003 | ¿Cuándo runtime necesita identidad de candle/timestamp para el body anchor y cuáles son las coordenadas para structural points distintos de los candidate HL/LH? | Equal body coordinates preserve one candidate price when only price is needed, but tie identity and other-point coordinates remain undefined. Bullish reset/invalidation membership now constrains the supplied turn; for a selected LH turn, the highest `Open`/`Close` body edge remains confirmed and separate from the sell SL wick anchor. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated equal-coordinate identity cases and direct coordinates for other structural-point classes |
...
- Exact-doji behavior and mirrored bullish/bearish terminal/reset membership are confirmed. Near-doji thresholds and remaining prior-HH/LL boundary inclusivity remain unresolved. Equal body coordinates do not make the selected price ambiguous when only price is needed, but candle/timestamp identity remains unresolved.
...
Broader structural states remain: `CANDIDATE_HL_SELECTION = PARTIALLY_DEFINED`; `CANDIDATE_LH_SELECTION = CONFIRMED` for highest-candidate identity within a recognized bearish correction; `BULLISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `BEARISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `TURN_MEMBERSHIP_SEGMENTATION = PARTIALLY_DEFINED`; `STRUCTURAL_CANDIDATE_DETECTION = PARTIALLY_DEFINED`; `NQ_H4_DIRECTION_BOOTSTRAP = PARTIALLY_DEFINED`. Mirrored terminal semantics and confirmed post-anchor direction/pair compression narrow these boundaries but do not resolve initial-anchor selection, historical scan termination, near-doji thresholds, remaining inclusivity, remaining structural coordinates or identity, mitigation, PDH/PDL ranking or ties, TP hierarchy/full-exit behavior, or the executable Stop Loss offset. All previously audited validation, exact-doji, origin-extreme precedence, body-anchor, protection-anchor and future-data states remain unchanged.

Still unresolved: deterministic first-anchor selection and historical scan termination, near-doji/small-body threshold, remaining prior-HH/LL boundary inclusivity, equal-coordinate body identity, other structural-point coordinates, mitigation semantics and the executable Stop Loss offset beyond the confirmed protection anchor. Exact `Close == Open` is neutral/no-start; both directions' in-range and terminal membership, causal ends and same-candle origin-extreme precedence are no longer open. The absence of a fixed lookback is confirmed; no fractal, ZigZag, N-left/N-right, ATR, percentage-swing, candle-count threshold, tolerance or fixed-point initialization may replace the missing first-anchor rule. Replay must not relabel the doji-time no-start decision, terminal membership or candidate HL/LH confirmation using future evidence; origin-extreme updates remain separate from structural HH/LL validation.
