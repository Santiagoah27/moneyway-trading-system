...
| NQ-Q-H4-005 | ¿Qué evento reproducible marca el primer miembro del vértice de origen H/L tras `StructureInvalidated`, y cuándo pertenecen al mismo vértice velas adyacentes con body prices distintos pero visualmente cercanos? | The final turning cluster excludes the incoming leg and ends before displacement; its selected extreme candle is core. Video 3 `08:45–09:20` shows these distinctions candle by candle on 5M, but neither an exact backward selector nor near-equal tolerance nor transfer of that detailed membership criterion to H4 is established. The conditional body/wick formulas remain confirmed. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Human-annotated H4 invalidation paths in both directions identifying every included/excluded origin candle, the first-member event, competing adjacent turning clusters and distinct near-equal body prices |
...
- `H4_PROVISIONAL_VECTOR_ORIGIN_MEMBERSHIP = PARTIALLY_DEFINED`: the origin is the final turning cluster rather than the whole incoming leg; its defining extreme candle belongs, earlier clean directional progress is outside in the reviewed 5M example, and the invalidating candle begins the new impulse outside the vertex. The exact backward boundary, visually near-equal membership and detailed 5M-to-H4 applicability remain unresolved. Exact equal body prices retain one level without a candle tie-break; they do not select the cluster. This is the remaining post-invalidation vector construction blocker; no fixed count, candle-color, liquidity-taking or numeric same-zone heuristic may fill it.
