...
- Bearish correction edge semantics, including dojis, gaps, exact candle inclusivity, confirming-candle scan participation and correction segmentation, remain partially defined. Within an already selected LH turn, the structural coordinate and bearish body anchor are confirmed as the highest `Open`/`Close` body edge among member candles.
...
| Question ID | Question | Why it matters | Current status | Blocking level | Proposed evidence needed |
|---|---|---|---|---|---|
| NQ-Q-LQ-001 | ¿Cómo se manejan dojis/gaps, candle inclusivity, confirming-candle scan participation y turn membership dentro de las correction windows; qué significa no mitigado y cómo se ordenan PDH/PDL frente a 1H/4H? | Bullish candidate-HL construction is partially defined; bearish causal validation/origin, complex highest-high candidate selection and the selected-turn highest-body-edge LH coordinate are confirmed. Window edges, mitigation and cross-class ranking remain unresolved or partial. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated bullish/bearish edge cases, other structural classes, mitigation boundary cases, PDH/PDL coexistence and ranking ties |
