...
| NQ-Q-H4-002 | ¿Qué threshold, si alguno, distingue un near-doji/small body; cómo se manejan gaps, la inclusividad exacta de los prior HH/LL boundaries y la membresía del turn? | Exact `Close == Open` is neutral and cannot start either correction, including when it updates an origin extreme. Both directional starts/ends and both same-candle origin-extreme precedence cases are confirmed. Near-doji and the remaining edge semantics are not reproducible. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Numeric near-doji evidence plus annotated bullish and bearish gap, boundary-inclusivity and turn-membership cases |
...
- Exact-doji behavior is confirmed. Near-doji thresholds, gaps, exact prior-HH/LL boundary inclusivity and turn membership remain unresolved. Equal body coordinates do not make the selected price ambiguous when only price is needed, but candle/timestamp identity remains unresolved.
...
Still unresolved: a reproducible viewport for the visual bootstrap, near-doji/small-body threshold, gap handling, exact prior-HH/LL boundary inclusivity, turn membership, equal-coordinate body identity, other structural-point coordinates, mitigation semantics and the executable Stop Loss offset beyond the confirmed protection anchor. Exact `Close == Open` is neutral/no-start; both directional causal ends and both demonstrated same-candle origin-extreme precedence cases are no longer open. No fixed-lookback, fractal, ZigZag, N-left/N-right, ATR, percentage-swing, candle-count threshold, tolerance or fixed-point initialization is supplied. Replay must not relabel the doji-time no-start decision or label candidate HL/LH as confirmed using future evidence; origin-extreme updates remain separate from directional correction start.
