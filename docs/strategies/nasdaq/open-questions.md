...
Broader structural states remain: `BULLISH_CORRECTION_START = CONFIRMED`; `BEARISH_CORRECTION_START = CONFIRMED`; `CANDIDATE_HL_SELECTION = PARTIALLY_DEFINED`; `CANDIDATE_LH_SELECTION = CONFIRMED` for highest-candidate identity within a recognized bearish correction; `BULLISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `BEARISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `TURN_MEMBERSHIP_SEGMENTATION = PARTIALLY_DEFINED`; `STRUCTURAL_CANDIDATE_DETECTION = PARTIALLY_DEFINED`; `NQ_H4_DIRECTION_BOOTSTRAP = PARTIALLY_DEFINED`. `PRE_START_WAITING_CANDLE_MEMBERSHIP = CONFIRMED_EXCLUDED_FROM_CORRECTION` covers every waiting candle, including after `ResetAndAwaitCorrectionStart`; `DISTINCT_EXTREME_CANDLE_MEMBERSHIP` and `PRE_START_EXACT_DOJI_AT_EXTREME` remain `CONFIRMED_EXCLUDED_FROM_CORRECTION`. Prior structural HH/LL equality and wick-only penetration are included in the active turn absent another terminal event; confirming body-close candles are excluded. Initial-anchor selection, historical scan termination, near-equal-zone tolerance, near-doji treatment beyond exact comparisons, other structural coordinates, mitigation, PDH/PDL ranking or ties, TP hierarchy/full-exit behavior and executable Stop Loss offset remain open.

Still unresolved: deterministic first-anchor selection and historical scan termination, near-equal structural-zone tolerance, near-doji/small-body threshold, other structural-point coordinates, mitigation semantics and executable Stop Loss offset beyond the confirmed protection anchor. Prior-HH/LL equality, wick-only penetration and their geometry membership are resolved; the prior structural validation reference remains distinct from the correction-origin reset extreme. Exact equal-coordinate body identity is one price level with no candle tie-break. Exact `Close == Open` is neutral/no-start; both directions' in-range and terminal membership, causal ends and same-candle origin-extreme precedence are no longer open. The absence of a fixed lookback is confirmed; no fractal, ZigZag, N-left/N-right, ATR, percentage-swing, candle-count threshold, tolerance or fixed-point initialization may replace the missing first-anchor rule. Replay must not relabel prior decisions using future evidence.
