...
| NQ-Q-H4-002 | ¿Qué exact boundaries delimitan la entire corrective phase y qué candles pertenecen al candidate-HL turn/base; existe evidencia directa para seleccionar candidate LH? | Candidate HL is the deepest low/base of the confirming impulse and internal pauses are rejected, but correction/turn segmentation and inverse candidate-LH selection are not reproducible. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated correction boundaries, turn membership, excluded intermediate lows/impulse pauses and direct candidate-LH evidence |
| NQ-Q-H4-003 | ¿Cómo se resuelve la identidad del body anchor cuando varios cuerpos comparten la coordenada extrema, y cuál es la coordenada para structural points distintos del candidate HL? | The selected candidate-HL turn uses its lowest Open/Close, but turn membership, tie identity and other structural-point coordinates remain undefined. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated multi-candle turns, equal-coordinate ties and direct coordinates for candidate LH/other point classes |
...
- Exact correction boundaries, turn membership and tie-breaking between bodies sharing the same extreme coordinate remain unresolved.
...
Current boundaries: `STRUCTURAL_VALIDATION_RULE = CONFIRMED`; `CANDIDATE_HL_SELECTION = PARTIALLY_DEFINED`; `CANDIDATE_LH_SELECTION = UNRESOLVED`; `CORRECTION_PHASE_BOUNDARIES = UNRESOLVED`; `STRUCTURAL_BODY_ANCHOR = PARTIALLY_DEFINED`; `BODY_ANCHOR_TIE_BREAKING = UNRESOLVED`; `STRUCTURAL_CANDIDATE_DETECTION = PARTIALLY_DEFINED`.

Still unresolved: a reproducible viewport for the visual bootstrap, exact correction/turn segmentation, candidate-LH selection, equal-coordinate body identity, other structural-point coordinates and mitigation semantics. No first-candle, first-two-candle, fixed-lookback, fractal, ZigZag, N-left/N-right, ATR, percentage-swing, candle-count threshold, tolerance or fixed-point initialization is supplied. Replay must not label the selected candidate HL as confirmed before the subsequent break closes; the confirming close changes status only from its own observable `AsOfUtc` onward.
...
| Question ID | Question | Why it matters | Current status | Blocking level | Proposed evidence needed |
|---|---|---|---|---|---|
| NQ-Q-LQ-001 | ¿Qué boundaries delimitan la corrección/turn de candidate HL, cómo se seleccionan candidate LH y otros structural points, qué significa no mitigado y cómo se ordenan PDH/PDL frente a 1H/4H? | Candidate-HL deepest-low/base selection and its lowest-body anchor are partially defined, while segmentation, inverse/other candidate selection, mitigation and cross-class ranking are not. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated correction/turn boundaries, candidate LH/other classes, body-anchor ties, mitigation boundary cases, PDH/PDL coexistence and ranking ties |
