...
| NQ-Q-H4-002 | ¿Cómo se manejan dojis/`Close == Open`, gaps, una bearish candle que simultáneamente crea un nuevo High en el lado bullish, la inclusividad exacta de los prior HH/LL boundaries y la membresía del turn? | Both directional correction starts/ends, the demonstrated bearish-side bullish-candle/new-low precedence, confirming-candle exclusion and the single-candle `Open` origin coordinate are confirmed. The symmetric bullish-side precedence case and remaining edge semantics are not reproducible. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated bullish and bearish doji/gap, bullish-side bearish-candle/new-high, boundary-inclusivity and turn-membership cases |
