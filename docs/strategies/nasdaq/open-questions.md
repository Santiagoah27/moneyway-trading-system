...
| NQ-Q-H4-005 | ¿Qué candle(s) delimitan el vértice de origen H/L del impulso opuesto tras `StructureInvalidated`? | Its body-edge StructuralPrice and wick ProtectionAnchor formulas are confirmed for a supplied vertex, which may contain multiple adjacent candles and ends before the displacement/invalidation candle. The exact first-candle boundary and full member set remain human-recognized rather than algorithmic. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated invalidation paths identifying the complete origin-vertex member set for both directions, including competing turning clusters |
...
- `H4_PROVISIONAL_VECTOR_ORIGIN_MEMBERSHIP` remains `UNRESOLVED`: the turning origin may include several adjacent candles and excludes the later displacement/invalidation candle, but no exact source-backed rule selects where the vertex starts. This is the remaining post-invalidation vector construction blocker; no fixed count, candle-color, equal-price or liquidity-taking heuristic may fill it.
