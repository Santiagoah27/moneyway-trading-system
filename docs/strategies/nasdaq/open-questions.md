...
| NQ-Q-H4-003 | ¿Existe alguna tolerancia numérica autorizada para fusionar body prices distintos en una structural zone y cuáles son las coordenadas para structural points distintos de los candidate HL/LH? | Exact equal coordinates form one price-level identity without candle ownership. Video 2 H4/1H evidence instead shows human visual grouping of nearby but distinct bodies, without a measured tolerance; no numeric merger may be inferred. Other-point coordinates remain undefined. For a selected LH turn, the highest `Open`/`Close` body edge remains confirmed and separate from the sell SL wick anchor. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Explicit human source for any proposed numeric tolerance, if intended, and coordinates for other structural-point classes |
| NQ-Q-H4-004 | ¿Cómo se corrige o reemplaza evidencia humana del bootstrap inicial? | The original assertion does not expire per session and post-invalidation trend change does not require a new arbitrary human anchor. Duplicate/conflicting anchors, reviewer correction or supersession of mistaken evidence, and missing-input runtime status remain unspecified. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Human-reviewed examples of competing or corrected initial assertions and a missing-anchor replay frame |
| NQ-Q-H4-005 | ¿Cómo se corrige o reemplaza una observación humana del mismo vértice post-invalidation y qué resultado se devuelve si falta la evidencia? | [ADR 0007](../../decisions/0007-define-nasdaq-human-origin-vertex-replay-input.md) defines the positive membership input; [ADR 0008](../../decisions/0008-define-nasdaq-human-origin-vertex-conflict-policy.md) confirms that equal member sets are compatible evidence and different visible sets block structural progression with `human_validation_required`. Canonical transport, selection, one-observation candle resolution and geometry calculation exist; explicit correction/supersession and zero-observation runtime mapping remain open. Initial raw-history bootstrap is separate. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Human-reviewed correction/supersession examples and a missing-evidence replay frame with a source-backed runtime result |
| NQ-Q-H4-008 | ¿La precedencia de vértice definitivo también aplica cuando una vela con gap migra el extremo provisional y cierra más allá del terminal congelado con cuerpo contrario o doji? | The reviewed `NQ-Q-H4-007` collision has a bullish validating body for HH or bearish validating body for LL. A strict validating `Close` alone does not imply that body direction if `Open` gaps beyond the reference. The wick migration and close break may still coexist, but the source does not establish whether this non-directional candle itself becomes the definitive vertex. | `UNRESOLVED` | `semi_automatic_backtesting` | Human-reviewed gap examples with a contrary or exact-doji body that both exceed the provisional candidate wick and close beyond the frozen terminal, identifying definitive vertex membership and validated output |

Confirmed or materially narrowed by the new human source review:

- `NQ-Q-H4-007` is resolved for the reviewed validating **directional-body** collision by human review of Video 3 approximately `07:40–07:46` and Video 2 approximately `03:20–03:40`: on the same closed H4 candle, strict candidate wick migration precedes strict frozen-terminal body-close validation. The confirming candle becomes the definitive HL/LH vertex with its own body coordinate and wick extreme, superseding the earlier provisional candidate without changing earlier replay frames. The first opposite-body candle's earlier provisional-formation body exclusion is a different event. Equal candidate anchors do not migrate; equal frozen-terminal closes do not validate. The narrower contrary/doji-body gap case is separately open as `NQ-Q-H4-008`.
- `NQ-Q-H4-006` is resolved by direct review of Video 2 approximately `12:45–13:45` and Video 3 approximately `07:40–07:46`: provisional HL migrates only on `Low < Candidate ProtectionAnchor`; provisional LH migrates only on `High > Candidate ProtectionAnchor`. Equality does not migrate, and no body, Close or tolerance comparison replaces the strict wick rule. Earlier provisional states remain causal replay evidence; the separately numbered same-candle collision is resolved in `NQ-Q-H4-007`.
...
- `H4_STRUCTURAL_STATE_LIFETIME = CONFIRMED`: an active HH+HL or LL+LH pair persists across trading sessions until a qualifying structural transition replaces or invalidates it. The formally closed 08:00 H4 candle is reviewed against that state; daily preparation does not rebuild it from raw candles or require a new human anchor. Asia/London extrema and the preparation assertion remain session-scoped separately.
- `H4_INITIAL_ANCHOR_LIFETIME = PARTIALLY_DEFINED`: a session boundary does not expire accepted bootstrap evidence, which remains auditable provenance even when a confirmed break normally rolls the active pair. Post-invalidation trend change requires no human rebootstrap; `H4_POST_INVALIDATION_REINITIALIZATION = PARTIALLY_DEFINED`, not broadly unresolved. Human-evidence correction/duplicate precedence and missing-input runtime status remain `UNRESOLVED`.
- Direct review of Video 2 approximately `09:20–10:55` and `12:10–12:40` confirms old-pair invalidation followed immediately by opposite intent and provisional bearish `H -> L` or bullish `L -> H`, then candidate LH/HL retracement and a second strict closed break of provisional `L`/`H` to validate the opposite pair. The invalidating candle is excluded from old-turn geometry and belongs to the new impulse; it is not automatically a validated opposite point. This transition can cross sessions and later evidence cannot rewrite its earlier frames.
- `H4_PROVISIONAL_VECTOR_COORDINATES = PARTIALLY_DEFINED`: source-backed body-edge `StructuralPrice` and separate wick `ProtectionAnchor` apply to selected provisional high/low vertices; the completed bearish impulse terminal uses minimum body edge as its later break reference and minimum `Low` as protection, with the bullish mirror using maxima. Different candles can supply body and wick extrema; equal body coordinates share one level. The invalidating candle is the first new-impulse member and can supply the terminal extreme if correction follows immediately. Later impulse candles may extend it.
- Direct review of Video 3 approximately `07:40–07:46` confirms `H4_POST_INVALIDATION_CORRECTION_START_EXTREME_COLLISION = CONFIRMED`: a first bullish-body candle in the emerging bearish context can make a new lower `Low`, update terminal body/wick geometry and begin the candidate-LH correction on that same close; the bearish-body/new-High bullish mirror uses the same dual role. The completed impulse terminal then freezes, including that candle's applicable contribution, while later correction candles do not move it. Existing start/geometry primitives are compatible with this supplied-state behavior.
- `H4_PROVISIONAL_VECTOR_ORIGIN_MEMBERSHIP = PARTIALLY_DEFINED`: the origin is the final turning cluster rather than the whole incoming leg; its defining extreme candle belongs, earlier clean directional progress is outside in the reviewed 5M example, and the invalidating candle begins the new impulse outside the vertex. Video 2 H4/1H review confirms visually grouped adjacent bodies may be close without exact equality; the backward multi-candle boundary and near-equal membership are `HUMAN_VALIDATION_REQUIRED`, while a numeric tolerance remains `UNRESOLVED`. Exact equal body prices retain one level without a candle tie-break; they do not select the cluster. A fixed count, candle-color, single-wick-extreme or numeric same-zone heuristic may not replace the reviewed members. ADR 0007 specifies the causal membership payload; ADR 0008 specifies duplicate/conflict semantics, while correction and missing-evidence handling remain open.
- [ADR 0007](../../decisions/0007-define-nasdaq-human-origin-vertex-replay-input.md) specifies the causal vertex-membership payload and [ADR 0008](../../decisions/0008-define-nasdaq-human-origin-vertex-conflict-policy.md) specifies exact-set compatibility and conflict blocking. The observation proves selection only; side comes from the verified transition. Canonical transport and exact one-observation member resolution exist, but the selector and full structural consumer do not. `H4_HUMAN_ORIGIN_VERTEX_SUPERSESSION` and `H4_MISSING_HUMAN_VERTEX_RUNTIME_STATUS` remain `UNRESOLVED`; neither a later observation nor a session boundary rewrites earlier `AsOfUtc` results.
...
- The lifecycle disposition after `ResetAndAwaitCorrectionStart` is `AwaitingCorrectionStart` for the still-valid structural side; it is distinct from `StructureInvalidated` after either prior-HL/LH body-close invalidation. Both have an empty turn, but only the former permits the existing lifecycle start overload on subsequent closed candles. The invalidation candle belongs to the new opposite impulse context without validating an opposite swing or pair; no same-side correction may restart from that invalidated state. With `Unique` reviewed origin membership, the origin geometry and initial terminal from the invalidating candle are specified; the opposite vector needs a separate immutable handoff because `AwaitingCorrectionStart` presupposes a valid structural side and contains no origin, terminal or event binding. Existing correction-start classification is reusable from supplied opposite-side references, but runtime does not yet compose the provisional handoff or subsequent pair validation. Initial raw-history bootstrap remains separate.
