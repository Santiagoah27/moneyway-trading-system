...
| NQ-Q-H4-002 | ¿Cómo se manejan dojis/`Close == Open`, gaps, la inclusividad exacta de los prior HH/LL boundaries y la membresía del turn? | Both directional correction starts/ends and both same-candle origin-extreme precedence cases are confirmed, together with confirming-candle exclusion and the single-candle `Open` origin coordinate. The remaining edge semantics are not reproducible. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated bullish and bearish doji/gap, boundary-inclusivity and turn-membership cases |
