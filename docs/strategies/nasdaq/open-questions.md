...
| NQ-Q-H4-005 | ¿Cómo se suministra y valida causalmente la pertenencia humana de las velas del vértice de origen H/L tras `StructureInvalidated`? | The final turning cluster excludes the incoming leg and ends before displacement; its selected extreme candle is core. Video 2 H4/1H shows that the backward boundary and near-equal grouping use visual judgment, not a reproducible numeric event. Given reviewed members, body/wick geometry and downstream lifecycle are deterministic; this input is narrower than initial raw-history bootstrap. | `HUMAN_VALIDATION_REQUIRED` | `semi_automatic_backtesting` | Human-reviewed H4 member sets in both directions, with observation time, corrections/conflicts and missing-evidence cases to inform a future causal replay-input contract |
