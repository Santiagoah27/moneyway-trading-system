...
| NQ-Q-H4-002 | ¿Cómo se manejan dojis/gaps, la inclusividad del HH/candle confirming, la participación del confirming candle en el scan y la membresía exacta del turn dentro de las correction windows? | Prior HH → first bearish candle → causal closed break materially narrows the bullish window. Prior-LL origin, causal LH validation, complex highest-high candidate selection and replacement are directly confirmed, but window edge semantics are not reproducible. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated bullish and bearish doji/gap, inclusivity, confirming-candle participation and turn-membership cases |
| NQ-Q-H4-003 | ¿Cuándo runtime necesita identidad de candle/timestamp para el body anchor y cuál es la coordenada para structural points distintos del candidate HL, incluido el LH confirmado por identidad? | Equal lowest body coordinates preserve a single candidate-HL price when only price is needed, but tie identity, turn membership and the structural LH/other-point coordinates remain undefined. The sell SL highest-wick anchor is a separate risk semantic, not evidence for the structural coordinate. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated multi-candle turns, equal-coordinate identity cases and direct target/comparison coordinates for candidate LH/other point classes |
...
- Bearish correction edge semantics, including dojis, gaps, exact candle inclusivity, confirming-candle scan participation and correction segmentation, remain partially defined. Candidate-LH structural price and bearish structural body anchor are not confirmed by the identity evidence.
- The sell risk example anchors Stop Loss above the highest wick/tail of the relevant LH. This does not establish the wick as the structural LH coordinate for target selection, reconstruction or comparisons.
...
- Doji/gap behavior, exact HH/confirming-candle inclusivity and turn membership remain unresolved. Equal body coordinates do not make the selected price ambiguous when only price is needed, but candle/timestamp identity remains unresolved.
...
Current boundaries: `STRUCTURAL_VALIDATION_RULE = CONFIRMED`; `LH_CAUSAL_VALIDATION = CONFIRMED`; `BEARISH_CORRECTION_ORIGIN = CONFIRMED`; `CANDIDATE_HL_SELECTION = PARTIALLY_DEFINED`; `BULLISH_CORRECTION_START = PARTIALLY_DEFINED`; `BULLISH_CORRECTION_END = CONFIRMED`; `BULLISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `CANDIDATE_HL_PRICE_SELECTION = PARTIALLY_DEFINED`; `CANDIDATE_HL_IDENTITY_SELECTION = UNRESOLVED`; `DOJI_BOUNDARY_BEHAVIOR = UNRESOLVED`; `COMPLEX_CANDIDATE_LH_SELECTION = CONFIRMED`; `HIGHEST_HIGH_REPLACEMENT = CONFIRMED`; `BEARISH_CORRECTION_WINDOW = PARTIALLY_DEFINED`; `CANDIDATE_LH_STRUCTURAL_PRICE = UNRESOLVED`; `BEARISH_STRUCTURAL_BODY_ANCHOR = CANDIDATE`; `SELL_LH_SL_ANCHOR = CONFIRMED`; `CORRECTION_PHASE_BOUNDARIES = PARTIALLY_DEFINED`; `STRUCTURAL_BODY_ANCHOR = PARTIALLY_DEFINED`; `BODY_ANCHOR_TIE_BREAKING = UNRESOLVED`; `STRUCTURAL_CANDIDATE_DETECTION = PARTIALLY_DEFINED`.

Still unresolved: a reproducible viewport for the visual bootstrap, doji/gap handling, exact HH/confirming-candle inclusivity, confirming-candle scan participation, turn membership, structural LH body-anchor/coordinate semantics, equal-coordinate body identity, other structural-point coordinates and mitigation semantics. No fixed-lookback, fractal, ZigZag, N-left/N-right, ATR, percentage-swing, candle-count threshold, tolerance or fixed-point initialization is supplied. Replay must not label candidate HL or LH as confirmed before the corresponding break closes; each confirming close changes status only from its own observable `AsOfUtc` onward.
...
| Question ID | Question | Why it matters | Current status | Blocking level | Proposed evidence needed |
|---|---|---|---|---|---|
| NQ-Q-LQ-001 | ¿Cómo se manejan dojis/gaps, candle inclusivity, confirming-candle scan participation y turn membership dentro de las correction windows; cuál es la structural LH coordinate distinta del wick de SL; qué significa no mitigado y cómo se ordenan PDH/PDL frente a 1H/4H? | Bullish candidate-HL construction is partially defined; bearish causal validation/origin plus complex highest-high candidate selection are confirmed. Bearish window edges, structural LH coordinate/body anchor, mitigation and cross-class ranking remain unresolved or candidate. | `PARTIALLY_DEFINED` | `semi_automatic_backtesting` | Annotated bullish/bearish edge cases, explicit structural LH target/comparison coordinate distinct from SL wick, other structural classes, mitigation boundary cases, PDH/PDL coexistence and ranking ties |
...
| Question ID | Question | Why it matters | Current status | Blocking level | Proposed evidence needed |
|---|---|---|---|---|---|
| NQ-Q-SL-001 | ¿Qué algoritmo identifica el structural 5M HL para buys y el structural 5M LH para sells? | The conceptual reference is confirmed, including the sell highest-wick/tail anchor after the relevant LH is selected, but deterministic swing selection is not. | `human_validation_required` | `semi_automatic_backtesting` | Annotated HL/LH selections and rejected alternatives |
| NQ-Q-SL-002 | ¿Qué offset exacto más allá de la wick estructural aplica, incluidos spread, maximum Stop and oversized-stop behavior? | The wick/tail side is confirmed for the selected relevant 5M HL/LH; for sells the source specifies the highest wick/tail of the relevant LH, but “below/above” is not an executable price. This does not define target-point coordinates. | `unresolved` | `paper_trading` | Approved quantitative offset/cost policy and boundary cases |
